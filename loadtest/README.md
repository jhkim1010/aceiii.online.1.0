...
| 2026-07-27 | **A-2b stress** (붕괴점 탐색) | 500→1000→1500→2000 VU | 35 | 20s※ | 20s※ | **81.7%** | ★붕괴점 발견 = **약 950 판매/분(16건per s)**. 성공 요청만 보면 P95 86ms. login_fail 0%(2000 VU 부팅 포함). ※한도 초과 후 전량 클라이언트 20s 타임아웃 |

### ★ Wave E — 300매장 분산 측정 (2026-07-27, 운영 유휴 확인 후 실행)

| RATE | 판매 P95 | 성공 → DB 저장 | 참여 매장 | 실패율 | dn 중복 | 락 대기 | PG 커넥션 |
|---|---|---|---|---|---|---|---|
| 25/s | **36.6ms** | 9,119 → 9,119 | 303 | 0% | 0 | 0 | 9 |
| 50/s | **37.5ms** | 18,143 → 18,143 | 303 | 0.03%※ | 0 | 0 | 10 |
| 75/s (목표) | **44.7ms** | 49,769 → 49,769 | 303 | 0% | 0 | 0 | 34 |
| 100/s | **57.3ms** | 36,299 → 36,299 | 303 | 0% | 0 | 0 | 22 |

※ 50/s 의 6건은 서버 오류가 아니라 keep-alive EOF (F-13).

**결론**: A-2b 에서 나온 950 판매/분(≈16/s) 벽은 **단일 매장 한정**이었다.
같은 총량을 300매장에 분산하면 **100건/s 에서도 P95 57ms**, 저장 누락 0, 채번 중복 0,
락 대기 0 이다. 목표 부하(75/s = 3,000터미널 × 1건/40초) 기준 **P95 44.7ms — 여유 6배**.
서버 load 는 최대 4.5/8코어, 운영 DB 커넥션은 측정 내내 1~4 로 무영향.
즉 남은 확장 리스크는 총량이 아니라 **한 매장에 트래픽이 몰릴 때의 재고 행 경합(F-10)** 이다.
...
### F-14 [높음] 매장 타임존과 채번 UNIQUE 인덱스의 기준 타임존이 다르다 — 판매 거부 가능

Wave E 정합성 검증에서 store 9 의 daily_number 가 같은 날 1부터 다시 시작한 것을 보고 추적.

- `reserveDailyNumber()` 는 **매장별 `stores.timezone`** 으로 "오늘" 범위를 정한다.
- 반면 F-1 로 만든 UNIQUE 인덱스는 타임존이 **하드코딩**돼 있다:
  ```
  UNIQUE (store_id, ((sale_date AT TIME ZONE 'America/Argentina/Buenos_Aires')::date), daily_number)
    WHERE activity_type='sale' AND daily_number > 0
  ```
- **운영 4개 매장 전부 `timezone = 'America/Bogota'`** (UTC-5). 인덱스 기준은 UTC-3 →
  두 기준의 하루 경계가 2시간 어긋난다.
- 사고 시나리오: 현지 22:00~24:00(Bogota) 판매는 앱에겐 "오늘"이지만 인덱스에겐 "내일"이다.
  그 번호(예: 50~60)가 인덱스상 내일 자리에 먼저 들어간다. 다음날 앱이 1부터 다시 세다가
  50 에 도달하는 순간 **UNIQUE 위반 → 판매 500 거부**.
- 현재 위험도: 최근 120일간 22~24시(Bogota) 판매 **0건**, 7월 총 판매 63건 → 아직 미발현.
  볼륨이 늘거나 야간 영업이 생기면 터진다.
- 부수 문제: 매장이 아르헨티나(ARS/AFIP)인데 타임존이 Bogota 라 티켓 번호가 현지 **02:00**
  에 갱신된다 — 업무 의미로도 틀렸다.
- 조치안:
  1. (즉시·간단) `stores.timezone` 을 `America/Argentina/Buenos_Aires` 로 교정 → 앱·인덱스 일치.
     ★운영 DML 이므로 사용자 승인 필요.
  2. (구조적) `sales.sale_day_local` (앱이 매장 tz 로 계산해 저장) + `(store_id, sale_day_local,
     daily_number)` UNIQUE 로 교체 → 매장별 타임존이 달라도 안전.

### F-13 [중] keep-alive 소켓 종료 경합으로 응답 없는 EOF

- Wave E 50건/s 구간에서 19,664 요청 중 **6건(0.03%)** 이 `Post ...: EOF` 로 실패.
  서버 로그에는 아무 흔적이 없다(요청이 서버 처리에 도달하지 않음). DB 저장 수는 성공 수와 일치.
- 원인: Node 기본 `keepAliveTimeout` 5초. 클라이언트가 재사용하려는 순간 서버가 소켓을 닫으면
  응답 없이 끊긴다. 3,000터미널이 keep-alive 로 붙는 환경에서는 상시 발생한다.
- 조치: `main.ts` 에서 `keepAliveTimeout=65s`, `headersTimeout=66s` 로 상향(프록시보다 길게).
...
**분당 판매 성공 건수 (스테이징 DB 실측)**

| 시각 | 성공 판매/분 | PG 커넥션 | 락 대기 | load |
|---|---|---|---|---|
| 12:48 | 501 | 2/9 | 0 | 0.90 |
| 12:51 | 954 | 1/10 | 0 | 1.12 |
| 12:52 | 771 | 1/15 | 0 | 1.35 |
| **12:53** | **229** | **61/61** | **56** | 0.89 |
| 12:58 | 228 | 61/61 | 59 | 3.99 |
| 13:04 | 135 | 61/61 | 60 | 8.38 |

- **붕괴점 ≈ 950 판매/분 (약 16건/초).** 넘어서는 순간 처리량이 4분의 1로 꺾이고 회복되지 않는다.
  꺾인 뒤 VU 를 2000 까지 올려도 처리량은 100~350/분 사이를 맴돈다 — 부하를 더 줘도 안 늘어난다.
- **원인**: `UPDATE products SET stock = stock - N` 이 같은 상품 행을 두고 직렬화된다.
  대기 이벤트가 `Lock/transactionid`(35) + `Lock/tuple`(25) 로 61 커넥션 중 60이 락 대기.
  락을 쥔 트랜잭션이 길어지며 pgbouncer 슬롯(pool_size 50 + reserve 10)이 전부 점유되고,
  뒤이은 요청은 커넥션을 못 잡아 `SequelizeConnectionAcquireTimeoutError: Operation timeout`
  (`sales-create.service.ts:353`) → 판매 500 으로 터진다. 즉 **락 경합이 pool 고갈로 번지는 연쇄**다.
- **CPU 는 병목이 아니었다**: 붕괴 시점 load 0.89 (8코어). 8까지 오른 건 붕괴 이후 재시도/큐 때문.
- **로그인은 끝까지 버텼다**: 2000 VU 부팅 포함 `login_fail 0%`. F-4 개선(structure 캐시 +
  `idx_user_roles_user`)이 유효했다는 뜻. 이제 병목은 로그인이 아니라 판매 쓰기 경로다.
- **회복성**: 부하 제거 후 **약 60초**에 락 대기 0, 커넥션 정상 복귀. 영구 잠김은 없다.
- **운영 무영향**: 측정 내내 운영 DB 커넥션 1/4 유지, watchdog 미발동.

**★ 수치 해석 주의 (과장 금지)**: 이 시나리오는 시드 구조상 **1개 매장 / 142개 상품에
3,000 터미널**이 몰린 극단이다(`select count(distinct store_id) from users where username like 'lt_vu_%'` = 1).
따라서 950/분은 **전체 시스템 용량이 아니라 단일 매장 상한**이다. 실제 목표(300매장 × 10터미널)는
매장·상품이 분산돼 경합이 훨씬 낮다 — 목표 용량 검증은 `burst-multistore.js` 다매장 시나리오로
따로 측정해야 한다(Wave E). 그래도 이 상한은 실재한다: 대형 매장이 성수기에 초당 16건을
넘기면 그 매장에서 판매 500 이 터진다.

**조치 완료 (B-0c, 2026-07-27 배포)** — 1~3 적용, 4 는 미착수
1. ✅ 재고 차감을 트랜잭션 **커밋 직전**으로 이동(`applyStockDecrements`) + productId 오름차순
   적용(교착 경로 제거) + 같은 상품 라인 합산
2. ✅ `SET LOCAL lock_timeout=3s` / `statement_timeout=15s` — 무한 대기가 커넥션을 물고 쌓이지 않게
3. ✅ 과부하(pool 획득 실패·55P03·40P01·40001·57014·53300)를 **503 + Retry-After: 2** 로 응답.
   500 이면 POS 가 판매 실패로 오해해 중복 판매를 시도한다. 과부하 503 은 텔레그램에서 제외
4. ⬜ 재고를 행 UPDATE 대신 이동 원장(append-only)+집계로 전환 — 미착수(구조 변경)

**조치 후 단일 매장 실측**

| 투입 | 처리 | 판매 P95 | 실패율 | 락 대기 |
|---|---|---|---|---|
| 20건/s | 19.8 | 57.8ms | 0% | 0 |
| 40건/s | 39.5 | 94.0ms | 0% | 0 |
| 60건/s | 44.9 | 251ms | 0% | 0 |
| 100건/s | 40.9 | **287ms** | **0%** | 0 |

**단일 매장 상한 16건/s → 약 41건/s (2.6배)**, 그리고 더 중요하게 **포화 후에도 붕괴하지
않는다** — 지연만 오르고 500 연쇄·pool 고갈·락 적체가 사라졌다. 저장 건수 = 성공 응답 수(전 구간).

### F-11 [중] 트랜잭션 2차 오류가 원인 에러를 가린다

- 붕괴 구간 500 중 다수가 `current transaction is aborted, commands ignored until end of
  transaction block` (`sales-create.service.ts:894`, 재고 증감 `Promise.all`).
- 실제 원인(락 타임아웃·경합 실패)은 먼저 실패한 문장인데, 로그에는 **뒤따라 실패한 문장**이 남아
  진단이 한 단계 멀어진다. `Promise.all` 로 병렬 실행하는 구조라 어느 문장이 먼저 죽었는지도 흐려진다.
- 조치안: 트랜잭션 내 병렬 쿼리 지양(또는 첫 실패를 즉시 잡아 원인 보존 후 rollback).
