...
**후속 조치 후보 (Wave B)**
1. 재고 차감을 트랜잭션 **후반**으로 이동 + 상품 id 정렬로 락 획득 순서 고정(교착·tuple 대기 감소)
2. `lock_timeout` / `statement_timeout` 을 짧게 두고 **빠른 실패 + 재시도** (락 대기가 pool 을 먹지 않게)
3. pool 획득 실패는 500 이 아니라 **503 + Retry-After** 로 — 클라이언트가 재시도 가능한 신호를 받게
4. 재고를 행 단위 UPDATE 대신 **이동 원장(append-only) + 집계** 로 전환하는 구조 검토 (경합 제거)
