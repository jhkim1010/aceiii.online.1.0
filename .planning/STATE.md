...
- **Phase 79 added (2026-08-17)**: 표 행 높이 30px 통일 — 사용자 요청. 실측: `FullTable`(AG Grid 래퍼) 기본값 `rowHeight=42` 가 **76개 파일**을 한 번에 좌우하는 지렛대이고, 개별 지정 **8곳**(28·30·30·30·34·36·48·`ROW_HEIGHT`)과 `AgGridReact` 직접 사용 **3곳**(FileTable·AccessLogsView·POS ProductListTable), MUI `<Table>` **96개 파일**(theme override 로 일괄 처리해야 함)이 남는다. MUI `<DataGrid>` 직접 사용은 0(전부 이관 완료). 선행 샘플 1건 적용 완료(`CajaFuerteOperationsTable`, app `199130d`). **2026-08-17 PLANNED — 3 plans / 2 waves**: `79-FINDINGS.md`(실측) + `79-01`(AG Grid 계열: 단일 출처 상수 `table-density.ts` + FullTable 기본값 → 76곳 자동 반영, 개별 지정 34/48/28 제거, 직접 사용 3곳) + `79-02`(MUI `<Table>` 테마 override — `spacing(3.5)`=14px → `spacing(1.25)`=5px, 96곳 일괄) + `79-03`(시각 회귀 확인 · 예외 근거 · CLAUDE.md 밀도 규약, `autonomous: false`). ★ 사용자 결정(2026-08-17): **POS 도 예외 없이 30px** — 그런데 실측해 보니 **POS 는 이미 30px**(`ROW_HEIGHT = 30`)이라 실질 변화가 없다. 다음: `/gsd-execute-phase 79`.
