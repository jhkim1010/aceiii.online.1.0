...
last_updated: "2026-09-17T02:50:11.912Z"
...
★ **요건이 두 번 좁아지며 막혀 있던 것이 풀렸다.**
  핸드오프: `.planning/HANDOFF-2026-08-21-d-감사트리거-타임캡슐.md`
  · "판매·재고 말고 **SKU 상태만** 되돌린다" → `stocks` 를 안 건드리니
    내가 "사용자 판단 필요" 로 세워 뒀던 회계 질문이 **사라졌다.**
    **막혔다고 느끼면 요건을 먼저 의심할 것.**
  · "가격·퍼미션 변경은 **반드시 audit_logs 에**" → 감사 누락이 우회 대상이 아니라
    고칠 결함이 됐다.
  · 근본 원인: `audit.interceptor.ts:81` 이 이전 상태를 **요청 본문에서 받는다.**
    890건 중 old_values 가 7건뿐이었고 가격은 한 건도 없었다.
  · 조치: **DB 트리거 7개** + 세션 GUC 행위자 + 권한 전용 Unit of Work.
    운영 배포 완료(빌드 #780, 무중단 1,101요청 실패 0).
  · ★★ 안전 검사 중 **내가 만든 결함**을 찾았다 — 권한 저장이 전체 삭제 후 재생성이라
    **무변경 저장에도 감사 1,280행**(운영 전체가 890행인데!). 레거시 codigos_tmp 가
    244MB 가 된 것과 같은 형태다. diff 로 바꿔 **247ms/1,280행 → 9ms/0행.**
  · codex 가 8건을 잡았고 전부 내 결함이었다. 가장 반복된 형태는 **"일부만 하고 끝냈다"** —
    세 번째부터 사람 눈 대신 **기계로 전수 확인**했고 그 뒤로는 안 걸렸다.

Phase 85 (scale-durability-structural-enforcement) — **W5·W6-A·W6-B 완결 · 남은 것은 W6-C**
...
| Phase 89 P07 | 40min | 2 tasks | 4 files |
...
- [Phase 89]: 89-07: buildQrUrl() 헬퍼로 QR URL 조립 통합 — b/pt 는 정수+양수일 때만 붙어 구 라벨 하위호환 유지, 대조군 실증으로 가드 필요성 확인
- [Phase 89]: verificar-esquema-phase89.sh 기본 검사 대상 = 사람이 고르는 PGTARGET 이 아니라 앱의 .env DATABASE_* 실접속 DB (오늘 벌어진 로컬/스테이징 어긋남 사고 방지)
- [Phase 89]: 대조군 실증 중 두 건의 검사 자체 결함(T6 무력화, CROSS_SQL 세미콜론으로 SQL오류가 0행으로 위장) 발견 즉시 수정 — require_int() 가드 도입
- [Phase 89]: 89-12: /register?ref= 프리필 — RegisterForm.tsx 에 useRouter+useEffect 1개만 추가, guestGuard/OTP/CUIT 무변경. 보상 문구 신규 추가 없음(referral_credits 0행)
- [Phase 89]: 89-12: 로그인 상태 CTA② 는 홈으로 튕김(GuestGuard router.replace('/')) — 89-08 CTA 문구에 반영 필요
...
Last session: 2026-09-17T02:50:11.907Z
