...
운영은 `statement_timeout = 30s` · `idle_in_transaction_session_timeout = 60s` ·
pgbouncer transaction pooling(`pool_size=50`).

★ **정정 (codex 지적 P2, 2026-08-21).** 처음에 나는 "300매장이 되면 60초 벽에 부딪힌다"
라고 썼는데 **틀렸다.** 두 군데가 틀렸다:

1. `idle_in_transaction_session_timeout` 은 트랜잭션의 **총 실행 시간**이 아니라
   **쿼리 없이 유휴로 열려 있는 시간**을 제한한다. 복원이 쉬지 않고 쿼리를 던지는 한
   60초를 넘겨도 이 설정으로는 끊기지 않는다.
2. **매장 수가 300개로 는다고 한 매장의 복원량이 커지지 않는다.** 복원량을 키우는 것은
   그 매장 자신의 누적 데이터다.

실제 위험은 따로 있다:
- **커넥션 장기 점유** — 한 매장 복원이 pool 의 커넥션 하나를 통째로 오래 잡는다.
  `pool_size=50` 에서 이건 처리량 문제이지 타임아웃 문제가 아니다.
- **개별 쿼리의 30초(`statement_timeout`)** — 배치 INSERT 하나가 30초를 넘으면 그 문장이
  죽는다. 배치 크기는 여기에 맞춰 정해야 한다.
- 트랜잭션 안에서 외부 I/O(MinIO 검증 등)를 하면 그때 **유휴가 생겨** 60초 설정에 실제로
  걸린다. 저장소 규약이 이미 금지하는 것과 같은 형태다.

현재 규모: 최대 매장이 sales 137 / stocks 1,634 — 지금은 어느 쪽도 문제가 아니다.
**착수 조건은 "매장 수" 가 아니라 "단일 매장의 행 수" 로 잡아야 한다.**
...
★ **`stocks` 를 어떻게 되돌릴지가 W6-C 의 핵심이다 (codex 지적 P2).**
"기존 데이터를 지운다" 로만 정의하면 `stocks` 단계에서 막힌다 —
`trg_stocks_immutable` 은 **BEFORE DELETE OR UPDATE** 라 둘 다 차단한다(운영에서 정의 확인).

길은 둘뿐이고, 둘 다 명시해야 한다:

| | 방법 | 성질 |
|---|---|---|
| (가) | **반대 부호 보정 행**을 append 한다 | 원장 규약을 지킨다. 이력이 남는다 — "되돌렸다" 는 사실 자체가 기록된다. 잔액은 `trg_stock_balances_apply` 가 같은 트랜잭션에서 맞춘다 |
| (나) | `SET LOCAL ventago.stocks_maintenance = 'on'` 후 DELETE | 가드 함수가 **의도적으로 열어 둔 유지보수 경로**다(정의에서 확인). 이력이 사라진다 |

2026-08-11 에 운영 더미 데이터를 지울 때 (나)를 썼고, 그때의 판단 근거는
"존재하지 않았던 −1,800 출고를 원장에 영구히 남기지 않기 위해" 였다.
**제자리 복구는 그것과 다르다** — 되돌리기는 실제로 일어난 사건이므로 (가)가 기본이어야
한다고 본다. (나)는 "그 매장의 그 기간 원장을 통째로 없던 것으로 한다" 는 뜻이고,
그건 회계적으로 다른 주장이다. **판단 필요.**
...
### 한 것 — W6-A · W6-B (배포됨, 빌드 #779 SUCCESS)

| | |
|---|---|
| 커버리지 강제 | `store-backup-coverage.spec.ts` — 인벤토리 대조 + **마이그레이션 컷오프** 이중 감시 |
| 커버리지 확장 | 백업 대상 **41 → 119개 테이블**. 새로 담기는 행: store 9 **4,978** · store 6 **1,539** |
| 자격증명 마스킹 | MP OAuth 토큰 · WooCommerce 키 · WhatsApp 토큰 · `branches.api_key` |
| 테스트 | **16건**. 감시가 실제로 잡는 것을 일부러 위반을 만들어 증명 |

### ★ codex 가 잡은 것 — 둘 다 내가 만든 결함이다

1. **[P1] 범위를 넓히면서 노출도 넓혔다.** "무엇이 빠졌나" 만 세고 **"무엇이 새로 나가나"**
   는 안 셌다. `SELECT *` 로 담는 순간 MP OAuth 토큰과 WooCommerce 시크릿이
   다운로드되는 JSON 과 서버 디스크의 평문 파일에 실리게 됐다.
   → **커버리지를 늘리는 일과 노출을 늘리는 일은 같은 동작이다.**
   그 조사에서 **W6 이전부터 새고 있던 `branches.api_key` 도 발견**했다.

2. **[P2] 테이블에는 컷오프 감시를 걸고 컬럼에는 안 걸었다.** 인벤토리 재생성을 잊으면
   새 민감 컬럼을 spec 이 모른 채 통과하는데 `SELECT *` 는 즉시 그 값을 싣는다.
   내가 테이블에서 막은 것과 **똑같은 구멍**이었다.

### 내가 스스로 잡은 것

- 마스킹 규칙을 spec 이 **복사**해 갖고 있었다 — 이 저장소에서 내가 반복해 경고한
  "두 곳이 들고 있으면 갈라진다" 를 내가 저질렀다. `redactBackupRows()` 하나로 합쳐
  **테스트가 검증하는 코드와 운영이 실행하는 코드를 같게** 만들었다.
- 이름이 **두 벌**이다 — raw SQL 은 `api_key`, ORM `toJSON()` 은 `apiKey`.
  한쪽만 지웠다면 ORM 경로는 **아무것도 안 지우면서 지웠다고 믿었을** 것이다.
...
| | 왜 안 했나 |
|---|---|
| **W6-C 제자리 복구** | 설계 결정이 남아 있다 — `stocks` 되돌리기를 (가) 반대 부호 보정 vs (나) 유지보수 플래그 DELETE 중 무엇으로 할지는 **회계적 주장이 다르다.** 사용자 판단이 필요하다 |
| **복원의 `tables` 처리** | 지금 복원은 옛 키만 읽는다. 새로 담긴 70개 테이블은 **백업에는 들어가지만 복원되지 않는다** — 즉 지금 상태는 "보존은 되고 되돌리기는 부분" 이다 |
| **W6-D MinIO manifest** | 객체 존재·해시 검증. 별건으로 크다 |

★ **지금 상태를 정확히 말하면**: 백업(내보내기)은 온전해졌고 자격증명은 막혔다.
  복원(되돌리기)은 **여전히 부분적이며 제자리 복구가 아니다.**
