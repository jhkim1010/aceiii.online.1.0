...
## 1-sexies. ★ 열려 있는 문제 — 코드마드레 뷰가 부모 자신의 PB 를 못 본다

사용자 질문 **"각 매장에서 위반건은 없는가?"** 로 발견. 나는 store 6 만 보고 일반화했었다.

### 전 매장 항등식 검사 결과

| 조합 | 결과 |
|---|---|
| variante 뷰 · 전 지점 합계 | 7개 매장 전부 **위반 0** (210 제품) |
| variante 뷰 · 지점 선택 | 7개 매장 전부 **위반 0** (279 제품×지점) |
| cod.madre 뷰 · 전 지점 합계 | 7개 매장 전부 **위반 0** (55 제품) |
| **cod.madre 뷰 · 지점 선택** | **매장 6 에 2건 위반 (±40)** ← 검증 안 했던 조합 |

```
sku 251843001  coolsistema(6)  Stock 316  formula 276  diff +40
sku 251843001  HELGUERA(16)    Stock  16  formula  56  diff −40
```

### 원인 (오늘 변경과 무관한 선재 결함)

movido `sale#28` (40장) 이 **부모 제품(123, `is_parent=true`) 에 기록**됐고, 재고 원장도
**부모 자신의 ProductBranch**(pb 137 −40 / pb 138 +40)에 쓰였다.

그런데 `getItems` 의 cod.madre `rowsJoin` 은 **자식 PB 만** 조인한다
(`JOIN products child ... JOIN "ProductBranch" pb ON pb.product_id = child.id`).
→ `r_stock` 은 그 ±40 을 못 본다.
반면 `movInSubquery` 의 `productScope` 는 `si.product_id = p.id` 를 포함한다
(Phase 35 에서 "단일 variant 제품은 parent.id 로 저장된다"며 의도적으로 추가).
→ **MOV± 는 40 을 센다.** 같은 행에서 두 컬럼의 모집단이 다르다.

### 영향 범위 (실측)

부모 자신의 PB 에 재고 이력이 있는 부모 제품 **11개**. 그중 합이 0 이 아닌 것 3개:

| 매장 | SKU | 현재 화면 Stock | 부모PB 누락분 | 포함 시 |
|---|---|---|---|---|
| 6 | `2542001` | 251 | +44 | 295 |
| 9 | `25193443001` | 39 | −10 | 29 |
| 9 | `25193545001` | 189 | −10 | 179 |

`251843001` 은 매장 전체로는 0 이라 전 지점 합계에서는 안 드러나고 **지점을 골라야** 보인다.

### 제안 (사용자 승인 대기)

cod.madre 뷰의 `rowsJoin` 이 **부모 자신의 PB + 자식 PB** 를 함께 집계하게 한다.
그러면 ① 이 항등식 위반이 닫히고 ② Phase 35 가 DEBUG 로그로만 알리던
"VENTA=0 MISMATCH"(단일 variant 제품의 판매가 부모 PB 에 있어 자식만 읽으면 0) 도 같이 풀린다.

★ 다만 **화면의 Stock 숫자가 3개 제품에서 바뀐다.** 그 44/−10 이 자식 PB 물량과 별개의
실물인지(= 더해야 하는지) 확인이 필요하다. 재고 수치라 임의로 바꾸지 않았다.
