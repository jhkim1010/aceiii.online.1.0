...
## 1. ~~★ 지금 열려 있는 문제 — 통합 view 의 우측 상세 패널~~ → **해결됨 (2026-08-07 오후)**

> 원인은 `getMatrix` 의 JS 빌드 루프가 지점별 행을 덮어쓴 것이었다. api #635 / front #574 로
> 배포 완료. 상세는 `73-NEXT-5.md` §1. 아래 원문은 진단 기록으로 남긴다.
>
> ★ 이 문서의 추정 중 틀린 것: "우측 상세" 는 Historial drawer 가 아니라 **Panel C(색상×talle
> 매트릭스)** 였고, 30 은 "net 이 아니라 ingreso" 라서가 아니라 **HELGUERA 한 지점 값**이라서 30 이었다.
> 정정(correccion) 은 `type='adjust'` 라 `STOCK_FLOW_EXCLUDE_SQL` 에 걸려 Hoy+ 에서 애초에 제외된다.

### (원문) 통합 view 의 우측 상세 패널
