...
> | 레거시 재고 리포트 폐기 (세 번째 정의 제거) | ✅ front #576 / api #645 — `73-NEXT-6.md` |
> | **리포트를 `v_product_hijo` 위로 리팩터** (§11 끝) | ⏳ 정확성 아닌 재발방지 — 다음 세션 |
> | **D (default variant 강제)** | ⏳ 별도 phase |
>
> ★ 2026-08-08 추가: `reportsStocks.service.ts` 가 **네 번째가 아니라 세 번째 정의**를 갖고
> 있었다(부호만 보고 집계 — 예약·이동·입고취소가 입고/판매에 섞임). Cockpit `getItems` 에
> 위임해 제거했다. 그 결과 `rowsJoin`/`productScope` 분기를 가진 곳은 이제 **`getItems` 하나**다
> — §11 끝의 리팩터 대상이 그만큼 줄었다.
