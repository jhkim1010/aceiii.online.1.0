...
### 0-6 / 0-7 — 브라우저 자동화로 실측 (2026-08-10 09:37 현지, 영업시간)

Claude in Chrome 으로 `app.coolsistema.com/nueva-venta` 를 실제로 열어 측정했다
(coolsistema · Caja 1 · Terminal 3 로 로그인된 세션).

**0-6 POS 진입 (웜 캐시):**

| 지표 | 값 |
|---|---|
| 총 리소스 요청 | **78건** (script 25 · xhr 33 · css 7 · link 3 · img 1 · fetch 3 · beacon 2 · 기타 4) |
| **API 호출(`newapi`)** | **33건** ← W3 "요청 수 감소"의 비교 기준 |
| 전송량 | 4.5KB (웜 캐시라 대부분 캐시 히트 — **콜드 캐시 측정은 별도 필요**) |
| DOMContentLoaded / load | 570ms / 1,027ms |

`beacon 2건`은 route-timing·web-vitals 계측이 실제로 나가고 있다는 확인이기도 하다(0-8 과 일치).
**API 응답 크기는 잴 수 없다** — 교차 출처(`newapi`)라 `Timing-Allow-Origin` 이 없어
`transferSize` 가 0 으로 가려진다. 전송량 비교가 필요하면 그 헤더를 먼저 추가해야 한다.

**0-7 탭당 소켓 — 간접값 +2:**

| 측정점 | POS 탭 열기 전 | 연 뒤 |
|---|---|---|
| API 5002 established | 6 | **8** |
| nginx 443 established | 16 | **18** |

두 지점 모두 **+2**로 일치한다. 다만 이 값을 "탭당 WebSocket 3개"(SPEC R1)와 곧바로
비교하면 안 된다:

- **5002 의 연결은 전부 `127.0.0.1` 이다** — nginx 업스트림 풀이라 브라우저 탭과 1:1 이 아니다.
  REST keep-alive(65초)까지 섞여 있어, 탭을 닫아도 즉시 줄지 않는다(8 → 7 → 8 로 흔들렸다).
- 운영 중 다른 사용자의 트래픽이 섞인다(영업시간 측정).
- 클라이언트 쪽 정확한 계수는 실패했다: socket.io 가 `transports:['websocket']` 이라
  폴링 핸드셰이크가 없어 **resource timing 에 잡히지 않고**, `WebSocket` 생성자 패치는
  **문서 로드 전에** 걸어야 하는데 주입 시점이 그보다 늦다.

**따라서 W2 게이트("서버 동시 연결 1/3 이하")의 비교 기준으로는 5002/443 델타를 쓰되,
"탭당 WS 개수"는 DevTools Network › WS 로 한 번 눈으로 확인하는 것이 여전히 필요하다.**
코드상 `io()` 호출부는 4곳이나 `TeamChatPanel` 은 패널을 열 때만 마운트되므로,
관측된 +2 와 모순되지 않는다.
...
| 0-6 | POS 진입 요청 수·전송량 | **✅ 부분 확보 (2026-08-10, 브라우저 자동화)** — 아래 | 콜드 캐시 재측정은 미완 |
| 0-7 | POS 탭당 WS 수 | **⚠️ 간접 측정만** — 아래 | 정확한 WS 수는 DevTools 또는 document_start 패치 필요 |
