...
**조치 (W1 관찰 기간 중 선행):** `/app/logs` 를 호스트 볼륨으로 빼거나, 집계값을
W1 의 `daily.jsonl` 로 밀어넣는다. 후자가 낫다 — 이미 오프사이트 사본까지 도는 경로다.
컨테이너 재생성이 필요하므로 **승인 후 배포창에서** 진행한다.
...
"슬롯 50"은 앱(coolsistema) 기준이다. 한편 앱측 상한은 `1 × 4워커 × pool.max 20 = 80`.
**80 > 50 은 위반이 아니라 설계다** — pgbouncer 가 다중화 지점이고 `max_client_conn=1000` 안에 든다.
