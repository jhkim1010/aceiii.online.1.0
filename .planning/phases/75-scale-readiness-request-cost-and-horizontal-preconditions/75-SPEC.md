...
- **Current:** 로그 실측 — `sync_outbox` claim **3,015ms** · `campaign_recipients` claim **2,171ms** ·
  ...
  Little's Law 상 3초 쿼리는 그 pool 슬롯의 처리량을 **1/30** 로 만든다.
  그리고 `slow_query_log` INSERT 가 2초라는 것은 **느린 쿼리를 기록하는 기능이 스스로 느린 쿼리**라는 뜻 —
  관측 장치가 관측 대상을 죽이고 있다.
...
- **Target:** 아래를 **전부 만족할 때만** `D-63-2` 를 재검토한다. 하나라도 미달이면 2호기는
  ...
  | G5 | `서버수 × 워커수 × pool.max ≤ pgbouncer 슬롯` | 예산 정합 |
