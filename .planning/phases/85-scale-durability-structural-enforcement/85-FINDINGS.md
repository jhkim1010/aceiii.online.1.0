...
- [x] `stocks` / `sales` / `sale_items` / `stock_balances` / `sync_outbox` 현재 행 수·크기
- [x] 운영 컨테이너 env 의 `TENANT_DERIVED_MODE` 실물값 (A-1 잔여)
- [x] 운영 `PGBOUNCER_POOL_SIZE` / `API_REPLICA_COUNT` / `WEB_CONCURRENCY` 실물값
- [x] 최근 7일 `cl_waiting` 피크 (`ops-metrics` samples)

---

## E-1. 실측 결과 (2026-08-19 23:30 ART / 운영 5434 · 읽기 전용)

### 행 수·크기 — W4 판정의 근거

| 테이블 | n_live_tup | n_dead_tup | 총 크기 | last_autovacuum |
|---|---:|---:|---:|---|
| `role_function_actions` | 3,645 | 0 | **9,080 kB** | — |
| `role_functions` | 11,790 | 0 | 3,336 kB | 2026-08-19 |
| `audit_logs` | 871 | 0 | 1,256 kB | — |
| **`stocks`** | **1,740** | 130 | **536 kB** | 2026-08-03 |
| `sales` | 163 | 0 | 392 kB | 2026-08-19 |
| `sale_items` | 664 | 22 | 184 kB | — |
| `stock_balances` | 448 | 37 | 184 kB | 2026-08-11 |
| `sync_outbox` | 0 | 0 | 48 kB | — |

★ **W4(파티셔닝)는 지금 착수하지 않는다 — 근거가 확정됐다.**
`stocks` 는 1,740행 / 536 kB 다. C-3 의 "3년 뒤 3억 행" 은 **300매장 가정에서의 투영**이고,
현재 실측과는 다섯 자릿수 차이가 난다. 지금 파티셔닝하면 얻는 것 없이 마이그레이션
복잡도·운영 절차·롤백 위험만 늘어난다. **파티셔닝은 테이블이 작을 때만 무통증**이라는
말은 "지금 하라" 가 아니라 "커지기 **전에** 하라" 는 뜻이며, 그 시점은 실 고객 유입으로
증가율이 관측되기 시작할 때다. 착수 트리거를 수치로 못 박는다:
**`stocks` 가 500만 행 또는 5 GB 를 넘거나, 주간 증가율이 50만 행을 넘으면 그때 W4.**

★ 뜻밖의 1위는 `role_function_actions` — 3,645행에 **9 MB**(행당 약 2.5 KB)다.
행 수 대비 크기가 비정상이라 bloat 또는 넓은 컬럼이 의심된다. 300매장이면 권한 테이블은
**매장 배수로 늘어나는 축**이라(현재 4매장 기준 `role_functions` 11,790행 → 매장당 약 2,950행,
300매장 환산 약 88만 행) 원장보다 이쪽이 먼저 커진다. **C-3 의 대상 테이블 선정이 틀렸다.**

### 운영 env 실물값

| 항목 | 실물값 | 판정 |
|---|---|---|
| `TENANT_DERIVED_MODE` | **`enforce`** | ✅ **A-1 잔여 해소** |
| `TENANT_GUARD_MODE` | 미선언 → 기본 `enforce` | ✅ |
| `API_WORKERS` | 미선언 → 4 (컨테이너 내 프로세스 4개로 실측 확인) | ✅ 단일 출처 작동 |
| `API_REPLICA_COUNT` | **미선언** — 로그가 `노드 1(미선언)` 로 표시 | ✅ 설계대로(경고가 보인다) |
| `SHOP_DB_ISOLATED` | **미선언** → 공개몰 상한 15 → **5 로 자동 축소** | ⚠️ 아래 |
| 커넥션 예산(로그) | 노드 1 × 워커 4 × (메인 20 + 공개몰 5) = **100 클라이언트** | — |

pgbouncer: `pool_mode=transaction` · `max_client_conn=1000` · `default_pool_size=20` ·
`reserve_pool_size=10` · **`ventago … pool_size=50`**.

★ **B-4 판정은 유지되지만 한 줄이 추가된다.** B-4 를 "해결" 로 뒤집은 근거가 `SHOP_DB_ISOLATED`
명시 플래그였는데, 운영에는 **그 플래그가 선언돼 있지 않다.** 안전장치는 설계대로 작동해
(미선언 → 비격리로 간주 → 상한 15→5) 예산은 지켜지고 있다. 다만 이것은 "격리했다" 가 아니라
**"격리 안 된 것으로 치고 좁혀 놨다"** 이다. 공개몰 트래픽이 늘면 이 5가 먼저 병목이 된다.

### 포화 지표 — 7일 (`ops-metrics` 5분 주기, 일 288 샘플)

| 날짜 | 샘플 | `cl_waiting` 최대 | `sv_active` 최대 | `sockets` 최대 |
|---|---:|---:|---:|---:|
| 08-14 | 288 | 0 | 7 | 7 |
| 08-15 | 288 | 0 | 4 | 8 |
| 08-16 | 288 | 0 | 0 | 0 |
...
`cl_waiting` 7일 피크 **1** — pgbouncer 큐에 실질 대기가 없다. `sv_active` 최대 10 은
`pool_size=50` 의 20% 다. **커넥션은 현재 병목이 아니다**(B-4 와 일치).

★ 단, 이 수치는 **4매장 트래픽**의 것이다. 300매장에서 의미를 가지려면 같은 지표를
**309매장 스테이징 부하 리그에서** 찍어야 한다 — 그것이 C-6·C-7 이 존재하는 이유다.
지금 이 표는 "여유가 있다" 가 아니라 **"아직 아무도 안 쓴다"** 의 증거로 읽어야 한다.

### 착수 순서에 미치는 영향

- **W4 (파티셔닝) → 보류.** 위 트리거 수치에 도달할 때까지 착수하지 않는다. 대신 C-3 의
  대상 테이블을 원장에서 **권한 테이블 계열**로 재검토한다(매장 배수로 먼저 커진다).
- **W1 (캐시 봉인) 부터** — FINDINGS 본문 결론 유지. 실측이 이 순서를 바꾸지 않는다.
- **A-1 종결.** `TENANT_DERIVED_MODE=enforce` 확인으로 잔여 항목이 없다.
