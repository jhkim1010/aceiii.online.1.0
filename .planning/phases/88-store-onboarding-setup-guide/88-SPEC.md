...
| **D-22** ✨**2026-09-12** | **업태(rubro)를 저장하지 않고, 응답에서도 뺀다.** 위저드 답은 **1회용 시드**로만 쓰고, 이후 분기는 기존 `use_restaurant_mode`/`use_size` 로 한다 | 사용자 결정. **CODEX P1** 이 「비식당 4종은 현재 상태에서 역으로 파생할 수 없다」고 짚었다 — `indumentaria`/`calzado` 는 시드된 사이즈 행만 다른데 그 행은 사용자가 고칠 수 있어 진실의 출처가 못 된다. ⤷ D-17 의 「저장하지 않는다」는 유지하되 **`rubro` 를 돌려준다는 약속을 철회**한다. 실측(운영 13개 `store_configs`): 구분되는 상태는 **3갈래뿐**(식당 2 · 사이즈 쓰는 소매 9 · 안 쓰는 소매 2). 「신발집인가」를 나중에 알아야 할 **용도가 아직 없다** — 상태를 늘리면 틀렸을 때 고칠 방법이 없다 |
| **D-23** ✨**2026-09-12** | **모드 전환 시 완료·「해당 없음」 승계 문제는 W3 착수 시점으로 미룬다.** `store_setup_steps` 의 키는 지금 그대로 | 사용자 결정. **CODEX P1**(소매에서 끈 `connect_printer` 가 식당 필수 단계에 적용됨 · 테이블을 지워도 `setup_tables` 완료가 남음)은 **모드별 카탈로그가 있어야** 실제 문제가 된다. 지금 카탈로그는 **8항목 고정**이고 식당 매장은 2곳이다. ★ W3 에서 모드별 카탈로그를 만들면 **그때 UNIQUE 키에 mode 를 넣는 마이그레이션이 필요하다** — 잊지 말 것 |
...
> ★★★ **이 §4 목록은 2026-09-09 판이라 낡았다.** 이름(`onboarding_*`)과
> D-12(기존 매장 숨김)가 그 뒤 뒤집혔다. **권위는 §3 결정표다.**
> 아래는 2026-09-12 기준으로 **실제 상태**를 덧붙인 것이다.
>
> | 웨이브 | 상태 |
> |---|---|
> | **W1 서버 계산기** | ✅ **완료·배포**(api #895). 이름은 `setup-guide`, 마이그레이션은 **아무것도 숨기지 않는다** |
> | **W2 홈 체크리스트** | ✅ **완료·배포**(front #729). 되살리기는 **홈**에 있다(Ayuda 메뉴는 안 만들었다) |
> | **W3 위저드** | ⏸ **미착수.** D-22·D-23 으로 전제가 바뀌었다 — 아래 참조 |
> | **W4 빈 상태 + 계측** | ◐ `<EmptyState/>` + 상품 목록 적용 · `scripts/setup-guide-metrics.sql`. **PostHog 이벤트는 안 보낸다**(아래) |
>
> ★ **W4 계측을 PostHog 로 보내지 않은 이유**: TTFV 와 이탈 단계가
>   **이미 `store_setup_events`/`store_setup_steps` 에 있다.** 같은 사실을 두
>   곳에 보내면 어느 쪽이 권위인지 흐려진다. 그리고 `posthog.service.ts` 의
>   규약은 「매장 업무 데이터를 외부로 보내지 않는다」다.
>   ⤷ 대신 **SQL 로 쟀다.** 그 쿼리는 **「표본이 없다」를 먼저 말한다** —
>     원장이 비면 모든 지표가 0 인데 그걸 「다들 잘한다」로 읽으면 정반대다.
>
> ★ W3 를 시작하기 전 남은 것: 모드별 카탈로그를 만들면 **`store_setup_steps`
>   UNIQUE 키에 mode 를 넣는 마이그레이션이 필요하다**(D-23).
