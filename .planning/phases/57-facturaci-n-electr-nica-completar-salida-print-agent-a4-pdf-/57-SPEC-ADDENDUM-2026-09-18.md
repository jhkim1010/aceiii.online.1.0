...
| **U7** | **superadmin 이 soap 으로 돌린다** | "모든 매장은 초기값으로 cool invoice gateway 를 사용… superadmin 권한으로 이를 soap 모드로 돌리도록" |
...
★★ **G3(PV 이전)이 이 요건에서 가장 무겁고 가장 값어치 있다.** 결번이
   사라지는 것이 아니라 **더 이상 새로 생기지 않는다.** 과거 결번은 `comprobante-externo`
   원장으로 계속 메운다(G7).

---

### R13 — superadmin 이 매장을 `ws` → `soap` 으로 돌린다 (U7)

**운영 모델이 확정됐다(2026-09-18):** 모든 매장은 **기본 `ws`(cool-invoice 게이트웨이)**
로 시작하고, **superadmin 만** 그 매장을 `soap`(ARCA 직접)으로 돌린다.
U5(게이트웨이 이탈)는 «전부 한꺼번에» 가 아니라 **매장 단위로 하나씩** 일어난다.

- **Current:** `afip_provider` 를 바꾸는 **API 가 없다**(실측 — 읽는 코드만 6곳).
  오늘 전환은 DB 직접 UPDATE 뿐이다.

#### ★★★ 함정 — provider 만 바꾸면 매장이 **조용히 시험 모드로 간다**

`ws` 에서 `afip_production` 은 **아무도 보지 않는 값**이다(게이트웨이가 무시하고 항상
운영 발급). 그래서 신규 매장 기본값 `false` 인 채로 방치된다.
`soap` 으로 바꾸는 **순간 그 값이 권위가 된다.**

| 무엇이 읽나 | ws 일 때 | soap 전환 직후 (`afip_production=false`) |
|---|---|---|
| 발급 경로 | producción | **producción** — W-A 백필이 `entorno='prod'` 를 넣어 막았다 |
| `afip-cert`(인증서 검증) · `serie-verificacion` · **`afip-reconciliacion`** · `reportes-arca` · `comprobante-externo` | producción | **homologación** ← 갈라진다 |

★★ **`afip-reconciliacion` 이 갈라지면 중복 발급이 난다.** 발급은 운영으로 나갔는데
   대조가 시험 환경을 뒤지면 → 운영의 미기록 CAE 를 「없음」으로 오판 → `liberar` 제안
   → **같은 판매를 다시 발급.** `entorno-afip.ts` 주석이 경고하는 바로 그 경로다.

- **Target:**
  1. **전환은 단일 동작이다** — `afip_provider` 만 바꾸는 경로를 만들지 않는다.
     한 번의 호출이 `afip_provider` · `afip_production` · 그 매장 **모든 발행자의
     `entorno`** 를 함께 정한다. 부분 전환 상태가 존재할 수 없어야 한다.
  2. **전환 전 점검(preflight)** — 통과 못 하면 전환하지 않는다:
     · 그 매장의 발행자마다 인증서가 있고 `parejaOk` · CUIT 일치 · `estado='activo'`
     · 인증서의 CA 환경이 **전환하려는 환경과 일치**(`esProduccion`)
     · `cert_slug` 가 공유 폴더(`AFIP_CERTS_SHARED_SLUGS`)가 아닌지 — **경고**(G1)
     ⤷ 실패 사유는 D-09b 와 **같은 4갈래**로 말한다(없음/CUIT불일치/만료/key짝).
  3. **미리보기 후 실행** — 무엇이 바뀌는지(발행자별 전·후 환경, PV, 인증서) 보여 주고
     superadmin 이 확인한 **그 상태를 되돌려 보내야** 실행된다.
  4. `soap` → `ws` **되돌리기도 가능**해야 한다. 되돌릴 수 없는 전환은 아무도 안 누른다.
  5. **감사 기록** — 누가 언제 어느 매장을 어느 방향으로 돌렸는지 `audit_logs` 에.
  6. superadmin **전용**. `assertOwnership` 으로는 부족하다 — 자기 매장 admin 이
     못 하게 막아야 한다(U7).
- **Acceptance:** `ws` 매장을 `soap`+producción 으로 전환하면 발급·인증서검증·
  번호열대조·reconciliación·보고서가 **전부 같은 환경**을 본다.
  인증서가 없는 매장은 **전환이 거절**되고 무엇이 없는지 말한다.
- **대조군 (셋):**
  · 전환 후 위 6개 읽기 경로의 환경이 **하나라도 다르면 실패**
    (발급만 확인하면 W-A 백필 덕분에 통과해 버린다 — 나머지 5곳을 따로 단언할 것).
  · 인증서 없는 매장의 전환이 **성공하면 실패**.
  · 매장 admin(비 superadmin)이 전환에 **성공하면 실패**.

★ **선행 작업:** 위 표의 6개 호출부 중 5개가 아직 `entornoProduccion(config)` 를 직접
  본다. R13 을 하려면 그것들을 **발행자 기준으로 옮기거나**, 전환이 `afip_production`
  까지 함께 맞춰 두 값이 갈라지지 않게 해야 한다. **둘 다 하는 편이 안전하다** —
  전환으로 값을 맞추고(즉효), 호출부를 옮긴다(구조).

★ 이 요건은 **U5 를 대체하지 않는다.** R13 은 «어느 매장이 soap 인가» 를 정하는 스위치이고,
  R12(G1~G4)는 «soap 인 매장이 게이트웨이와 무엇을 공유하는가» 를 없애는 일이다.
  R13 없이 R12 만 하면 전환할 방법이 없고, R12 없이 R13 만 하면 전환해도 여전히 얽혀 있다.
...
> ★★★ **2026-09-19 실측으로 이 절의 전제 셋이 틀린 것으로 확인됐다.**
> 상세: `.planning/ANALISIS-2026-09-19-phase57-게이트웨이이탈-F1F5-실측.md`
> 핵심: **TA 는 CUIT 이 아니라 인증서(CEE=DN) 단위다**(ARCA 기술명세 원문).
> 그래서 인증서를 «복사» 해도 결합은 안 풀린다 — **전용 인증서를 새로 발급**해야 한다.
> 아래 F3 절차(복사)는 그 점에서 낡았다. 아래 F4 의 「캐시 전용 파일」도 마찬가지다.


| # | 작업 | 결합 |
|---|---|---|
| F1 | ✅ **완료** — `resolvePvAndCoolUser` 는 **이미 있었고** 죽어 있었다(호출부 0건). 제거 | G5 |
| F2 | ✅ **완료** — 읽기를 `cert_slug` 로. 발급 경로가 그것을 **안 보고 있었다**(구멍 3개) | G4 |
| F3 | ✅ **코드 완료 / 운영 전환 보류** — 본체는 이미 되어 있었다. 남은 것은 폴더 **하나** | G1 |
| F4 | ⛔ **보류 — 이유가 바뀌었다.** 원인은 폴더가 아니라 **같은 인증서(CEE)**. 해법은 캐시 분리가 아니라 **Ventago 전용 인증서 발급** | G2 |
| F5 | ✅ **완료 — 단, 설정 시점 차단** (사용자 확정 2026-09-19). 발급 차단은 store 6 운영을 멈춘다 | G1 |
