...
```
k6 run ... -e RATE=25 -e DUR=2m -e PER_STORE=5 burst-multistore.js
```

| RATE | checks | http_req_failed | sale_ok | P50 | P95 | 중단 iteration |
|---|---|---|---|---|---|---|
| 10/s | **100%** (1,879/1,879) | **0%** (0/2,079) | 1,829 | 42.6ms | **64.4ms** | 0 |
| 25/s | **100%** (4,694/4,694) | **0%** (0/4,994) | 4,619 | 46.6ms | **79.2ms** | 0 |

### 두 회차 누적 DB 정합성

| 검증 | 결과 |
|---|---|
| BURST 총 저장 | **6,435** (= 1,829 + 4,606) |
| `daily_number` 중복 | **0** |
| 부분 저장(아이템 없는 판매) | **0** |
| **관측된 최대 초당 판매** | **29건/s** |

→ 요구치(5건/s)의 **약 6배**까지 올려도 저장 실패·중복·부분 저장이 전부 0.
지연은 25/s 에서도 P95 79ms 로 목표(300ms) 대비 3.8배 여유.
