...
**정책 확정 (사용자, 2026-07-28): 각 매장은 반드시 자기 상품만 판매한다. 혼합 운용은 절대 없다.**
→ 위 10건은 **정책 위반 잔존 데이터**(테스트기 유입)이며, 차단은 예외 없이 적용한다.
...
후속 조치(정책 확정에 따른 강화, 2026-07-28):
- `processSaleItems` 의 `storeId` 폴백 제거 — storeId 없이 호출되면 **400 으로 거부**한다
  (선택 인자로 두면 호출부 누락 시 스코프 없이 조회돼 경계가 조용히 뚫린다).
- **식당 모드 주문 경로(`restaurant-sale.service.ts placeOrder`)에 검증 신규 추가** —
  이 경로는 종전에 `dto.items[].productId` 를 **검증 없이** 그대로 `sale_items` 에 넣고 있었다
  (소매 경로만 고쳤다면 식당 모드로 경계가 그대로 뚫렸을 것). 단일 쿼리로 전량 확인 후 400.
- 기존 잔존 10건은 과거 데이터로 남긴다(소급 삭제/수정 없음). 필요 시 별도 정리 과제.
