# Phase 53: VentaGO 파트너 네트워크 - Research
...
- **D-01:** partner_links 대칭 구조(a<b 정규화, initiator 구분, status enum, 역할은 이벤트 시점 파생).
- **D-02:** shared_categories 매칭 키 = SKU/category_id/codigo_madre 중 db 대조 후 확정.
- **D-03:** 초대→수락(양측 독립 공유범위)→해제(shares 삭제). 매장 식별자 IDOR 방지.
- **D-04:** 캡처 = sales-create 커밋 후 best-effort 블록(트랜잭션 밖).
- **D-05:** 전달 = Phase 43 outbox 재사용(신규 pool 금지, 같은 DB 내 직접 INSERT).
...
### Claude's Discretion
- 매칭 키(SKU/codigo_madre/category_id) db 대조 후 PLAN 확정.
- sync_outbox 확장 vs partner_share_outbox 신규.
- rollup 저장 방식(테이블 vs 온디맨드+캐시).
- 감사 로그 저장소(activity_ledger/audit_logs 재사용 vs 전용).
...
연결 인프라(양측 동의·비대칭 공유·해제 이력삭제) / 판매 캡처+공유(판매 영향 0) / 프라이버시 하드가드(금액·고객 부재·감사) / 실시간 도매 대시보드(격리) / 알림+생산추천 / 소매 혜택. 상세는 53-SPEC.md.
</phase_requirements>
...
- **결론:** op_type 에 `'partner_share'` 추가로 흡수 가능. 단 partner_share 는 channel_id/platform 무관 → 컬럼 nullable 여부 확인 후 흡수 vs `partner_share_outbox` 신규 결정. cron worker 는 재사용.
...
### 6. 매칭 키 — PLAN 에서 확정 필요 (OPEN)
`sale_items`(product_id/quantity/promo 컬럼), `products`(sku/codigo_madre/category), `product_branches`(재고) 컬럼을 db-schema-tables.md 로 최종 대조 후 매칭 키 확정. cross-store 동일 SKU 전제 검증 — 불일치 시 MVP 는 수동 매핑 or SKU 문자열 매칭.
</codebase_findings>
...
- **전달 = outbox 큐 (BullMQ/Redis 신규 아님):** Phase 43 이 이미 트랜잭션-안전 outbox + cron worker 를 Sequelize 단일 pool 위에 구축. Redis/BullMQ 도입 = 신규 인프라·pool. cross-store 가 같은 DB 내이므로 외부 큐 불필요. **재사용이 pool 규약·운영 단순성 모두 우위.**
...
- **매칭 키 불일치:** cross-store SKU 상이. → PLAN 에서 db 대조, MVP 동일 SKU 전제/수동 매핑, 표준화는 Deferred.
