...
## CODEX 5차 — P2 1건 고침 · P1 1건 **미해결로 남김**

**[P2 고침]** 관문이 `Number(v) > 0` 만 봐서 **음수 할인·운임·세금이 조용히 0 으로
덮어써졌다** — 그 관문이 막으려던 동작이 그 자리에 그대로 남아 있었다. 커밋 `853b72ab`.

**[P1 미해결] 현금 기록과 서랍 판정이 서로 다른 조회를 쓴다.**

- 기록: `registerCashOperation` → `resolveOpenCashRegister`(지점 우선 + **사용자 폴백**, **잠금 없음**)
- 판정: `assertCanLand` → `getOpenCashRegister`(지점 **전용**, **FOR UPDATE**)

★ 내 분석(**검증 안 함** — 다음 세션이 확인할 것): 회수 판매 경로에서는 fail-closed 로 보인다.
  `assertCanLand` 가 `branchId` null 을 거부하므로 회수 판매는 branchId 가 항상 있고,
  그러면 두 조회가 같은 지점 세션을 고른다. 자동 마감이 기록과 판정 사이에 끼어들면
  판정의 잠긴 조회가 아무것도 못 찾아 **트랜잭션 전체가 롤백**된다(돈이 남지 않는다).
  ⤷ CODEX 가 말한 「이미 정산된 서랍에 현금이 남는다」는 **판정이 없는 일반 efectivo 판매**
    에 해당하고, 그건 이번 변경 이전부터 있던 것이다.

★★ 그래도 **조회가 둘인 것 자체가 결함**이다 — 같은 질문("어느 서랍인가")에 두 구현이
  답하면 언젠가 갈라진다(이 저장소에서 반복된 형태). 고치려면
  `registerCashOperation` 도 잠그고 지점 전용으로 옮겨야 하는데, 그 함수는
  **모든 efectivo 판매**가 쓰므로 영향 범위가 POS 전체다. 컨텍스트 79% 에서 손대지 않았다.
  ⤷ 다음 세션의 판단 대상. 먼저 위 「fail-closed」 추론이 맞는지 **실제로 재 볼 것.**

---

# ⑤ 완료 (2026-09-17) — POS 에서 `dp` 로 회수한다

커밋 `ventago-app 5b3f867` + `api-ventago 597bdd21`. 서버는 ② 에서 끝나 있었고 이번은 화면이다.

## 흐름 (구현된 것)

```
SKU 칸에 dp 또는 dpago → (힌트 배너 + ENTER)
  → 「Cobro de deuda」 다이얼로그: 빚 잔액 · 금액(전액 기본, 빚 초과 차단, 「Quedará …」)
                                 · Nº recibo(자동 생성, 수정 가능, 40자 상한)
  → 카트에 `dpago` 줄 (SKU 칸·제품 칸·cantidad 잠김 + 이유 표시)
  → 기존 결제 모달: credito/favor/senia 숨김 · 수단 하나만 · 할증 수단 거절
  → F2 → POST /sales { items: [], deudaPago: { storeClientId, receiptNo } }
```

## 결정 — 왜 이렇게 했나

**판정은 `ventago-app/src/views/homes/utils/deuda-pago.ts` 하나에만 둔다.**
화면 넷(SKU 칸 · 카트 · 결제 모달 · 확정)이 같은 규칙을 봐야 한다. 각자 조건문을 들고
있으면 한 곳만 고친 날 나머지가 조용히 우회로가 된다(이 저장소에서 반복된 형태).
목록은 **서버가 거절하는 것과 같다** — 프론트는 400 을 미리 알려 주는 역할만 한다.

**회수 줄을 카트 줄로 담는다(별도 상태로 두지 않는다).**
합계·결제 패널·영수증 미리보기·draft 자동저장이 전부 `products` 를 보고 돈다. 별도 상태면
그 넷을 다시 배선해야 하고, 무엇보다 **새로고침에 사라진다** — 영수증 번호가 사라지면
제출마다 새 번호가 생겨 두 UNIQUE 를 **둘 다 통과**하고 회수가 복제된다(② CODEX P1).
`SaleProductsContext` 의 draft 는 `products` 를 그대로 저장하므로 **아무것도 안 하면 맞는다.**

**금액은 다이얼로그에서만 정한다.** 표 인라인 편집·행 편집(`handleEditProduct`)은 회수 줄에서
거절한다 — 행 편집 경로는 줄을 카트에서 **지우고** 입력칸으로 옮기는데 입력칸은 회수를
모른다(영수증 번호와 고객이 그 자리에서 사라진다). 빚 한도와 남을 잔액을 같이 보여 주는
자리는 다이얼로그 하나다.

**영수증 번호는 `INT-MMDD-XXXX` 난수.** 순번은 서버가 발급해야 하고, 그러려면 회수를 시작하는
순간 번호를 예약해야 한다 — 취소하면 결번이 남는다. 종이 영수증이 있으면 사용자가 덮어쓴다.

## 거절 목록 (조용히 고치지 않는다)

물건 혼입(양방향) · 영수증 번호 없음/40자 초과 · **줄을 담은 뒤 고객이 바뀜** ·
할인/할증/운임(음수 포함) · 결제수단이 하나가 아님 · `credito`/`favor`/`senia` ·
수단 할증 · 결제액 불일치 · 보류(suspender) · F10(AFIP 발급) · 회수 판매 수정 진입.

★ 「줄을 담은 뒤 고객이 바뀜」이 통과하면 **남의 빚이 깎인다** — 되돌리는 경로는 없다.
  그래서 카트 줄이 `deudaStoreClientId` 로 출처를 들고 있다(확인 화면의 판정만으론 부족하다).

★ 할인을 0 으로 덮어쓰지 않는 이유: ②에서 서버가 조용히 덮어쓰다가 플래그가 잘못 붙은
  **정상 판매의 상품·재고가 통째로 사라진** 적이 있다(CODEX P2). 화면도 같은 원칙이다.

## 인쇄 — 건드리지 않았다

회수 판매는 AFIP 를 안 타므로(③) 무인 출력 경로는 `printTicket` 하나만 남고, 일반 판매와
**같은 횟수**다. print 관련 코드는 한 줄도 바꾸지 않았다.

## 검증

| 무엇 | 결과 |
|---|---|
| 신규 spec `deuda-pago-regla.spec.ts` | **42/42** (1차 27 + CODEX 대응 15) (대조군 포함 — 보통 SKU·efectivo·정상 판매가 통과하는지) |
| 돌연변이 4개 | 전부 잡힘 (아래) |
| app tsc | 0 |
| eslint (추가 파일 전부) | 0 (`PaymentSummaryModal` 경고 1건은 HEAD 에도 있다 — 대조로 확인) |
| api jest `src/app/sales` | 17 suite / 319건 |
| `next build` | **exit 0** (운영 빌드와 같은 경로) |

```
고객 일치 검사 제거        → 2건 실패 ✅
할인 !== 0 → > 0 (음수 통과) → 1건 실패 ✅
결제수단 개수 검사 제거     → 1건 실패 ✅
토큰 판정 startsWith('dp') → 1건 실패 ✅   (원복 후 27/27)
```

★ **화면 실측은 아직 못 했다** — 로컬 앱이 로그인 화면이고 비밀번호는 내가 넣지 않는다.
  로컬 DB 에 운영과 같은 고객(store 6 · storeClient 7470 · 외상 267,600)이 있어 그대로
  재현된다. 로그인만 되면 `dp` → 다이얼로그 → 카트 → 결제 → F2 를 눌러 확인할 수 있다.
  **빌드 통과는 배선 검증이 아니다** — 이 확인 전에는 "돌아간다" 고 말하지 않는다.

## CODEX 1차 — P1 1 · P2 2 · P3 1, **전부 맞았다** (커밋 `f2579d3`)

| 등급 | 지적 | 판정 | 조치 |
|---|---|---|---|
| P2 | 채무 조회 **실패·로딩·0** 이 금액 상한을 fail-open 시킨다 | 맞다 (내가 스스로도 같은 결론에 도달했다) | `resolveCobroLimit()` 로 빼내 그 다섯 경우를 전부 **모른다**로 접는다 |
| P2 | 회수 줄이 **지점**을 안 들고 있다 — draft 키에 지점이 없어 A→B 전환 후 복원하면 다른 서랍에 기록된다 | 맞다 | 줄에 `deudaBranchId`, 확정 때 대조. 지점 미확정이면 시작 거부 |
| P1 | 영수증 번호가 **영속됐다는 보장이 없다**(1초 디바운스) | 맞다 | `saveDraftNow`/`readPersistedDraft` 노출 → 줄 생성 시 즉시 저장 + 확정 직전 **되읽어 확인**, 불일치면 거부 |
| P3 | 난수 접미사 4자(105만) — 하루 500건이면 충돌 11% | 맞다 | 6자(10.7억). 15자라 varchar(40) 안 |

★★ **P2 「상한이 사라진다」는 이 저장소에서 반복된 형태다** — 좁히는 조건이 해석 실패에서
  전체로 폴백하는 것. 서버는 빚을 넘는 금액을 거절하지 않고 초과분을 `favor_in`(선납)으로
  기록하므로, 화면이 막지 않으면 캐셔가 **의도 없이 고객 예치금을 만든다.**

★ **거부가 막다른 길이 아닌지 확인했다.** 영수증 번호를 저장하지 못하면 회수를 거부하는데,
  회수를 등록하는 **다른 길이 있다**(`/cuentas-corrientes` = `POST /credit/payments`).
  복제 위험이 있는 길만 닫는 것이고, 기능 자체가 죽는 것이 아니다.

★ 곁가지로 결제액 비교를 `!==` → **0.005 관용**(서버 `redondearMonto` 와 같게)으로 바꿨다.
  좁히는 쪽이 서버보다 좁으면 **서버가 받아 줄 정상 회수를 화면이 거절**한다 — 그것도 결함이다.

**유보(서버 몫)**: CODEX 는 서버도 회수 시점 미수금 한도를 **잠금 아래 재검증**해야 한다고
했다. 맞지만 초과분 → `favor_in` 은 `/credit/payments`(선납)와 **공유하는 의도된 동작**이라,
바꾸면 그 경로의 업무 규칙이 같이 바뀐다. ⑥ 과 함께 판단할 것.

**돌연변이 5개로 새 판정을 다시 쟀다** — 전부 잡혔다:
상한 fail-open 되살리기 → 4건 실패 / 지점 대조 제거 → 2건 / 번호 영속 확인 제거 → 2건 /
금액 관용 0 → 1건 / 난수 4자 → 1건. 원복 후 **42/42**. `next build` exit 0.

## 곁가지로 알게 된 것

**CODEX 자동 훅의 자격증명 탐지가 오탐을 낸다.** `isDpToken = isDeudaPagoToken(skuText)` 가
`SECRET_RE` 의 `token['"]?\s*[:=]\s*…` 에 걸려 **프론트 커밋의 검토가 건너뛰어졌다**
(메시지는 남으므로 조용하지는 않다). 손으로 diff 를 넘겨 검토했다.
⤷ 값이 **인용된 리터럴**일 때만 걸리게 좁히는 것이 맞아 보이지만, 좁히는 쪽이 위험한
  변경이라 이번 범위에서 손대지 않았다. `Token`/`Secret` 이 든 식별자를 대입하는 코드는
  앞으로도 같은 이유로 건너뛰어진다.
