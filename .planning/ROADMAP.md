...
### Phase 80: 생산 중(WIP) 수량·완성 예상시점을 Stocks 리포트에 (talleres WIP visibility)

**Goal:** Stocks 리포트에서 **"이 제품이 지금 몇 장 생산 과정에 있고, 언제쯤 나오는가"** 를
권한 있는 사용자만 볼 수 있게 한다. 산출물은 숫자 두 개가 아니라 **그 숫자가 사실인지 추정인지
화면이 스스로 말하는 상태**다.

**계기:** 사용자 요청(2026-08-17).

**★ 사용자 결정 (제안과 다름):** 생산 중 물건에는 **지점이 없다**(지점은 수령 시
`talleres_recepciones.target_branch_id` 로 처음 정해진다). 나는 "전 지점 합계만" 을 권했으나
사용자가 **지점 수 균등 분배**를 선택했다. → 받아들이되 **추정치임을 필드명·화면에 못박는다**
(`estimatedBranchWipQty` · `allocationMethod` · 값마다 `≈`).

**Requirements:**
- R1 **WIP 수량** — 총량/공방 내/공정 사이/재고 미반영을 **구분해서** 노출
- R2 **ETA** — 최종 공정만 완성예정일, penúltima 는 "다음 이정표", 나머지는 미정, 지연은 OVERDUE
- R3 **지점 균등 분배** — 결정적 규칙(잔여 배분 포함) + 합계 보존
- R4 **권한** — 매장 admin 이 사용자별 부여, **서버가 필드 단위로** 차단(정렬·export 포함)
- R5 **모집단 정합** — CodigoMadre/variant 뷰의 기존 모집단(`groupLeafIds`)과 일치

**설계 근거·함정:** `80-FINDINGS.md`(실측) + `.gsd/review-codex-phase80-talleres-wip.md`
(CODEX 검토 — 내 초안에서 **Blocker 6건**을 잡았다: WIP 정의 누락분 · 로트당 단일 공정 가정 ·
`max(due_date)` 오용 · 엔드포인트 단일 guard 의 필드 유출 · 분배 규칙 미고정 · madre/leaf 모집단).

**범위 밖:** 공정별 표준 리드타임(완료 로트 3건뿐 — 표본 없음) · 계획 배분 입력 UI(W3) ·
`talleres_etapas` 스키마 변경 · Flutter 앱 화면

**Success Criteria** (what must be TRUE):
- 지점 행들의 합이 **항상** TOTAL 과 정확히 일치 (1000/3 → 334+333+333)
- 권한 없는 사용자의 응답에 WIP 필드가 **아예 없다** (열 숨김이 아니라 필드 부재) — 정렬·export 도 거부
- stocks 원장 행이 100행인 제품과 0행인 제품의 WIP 값이 같다 (CTE 복제 없음)
- 분할 발송·rework 로 여러 공정에 동시에 있는 로트에서 일부 cohort 의 상태가 전체로 번지지 않는다
- 지난 due_date 가 미래 완성예정일처럼 표시되지 않는다
- 기존 `/reports/stocks-cockpit/items` 응답의 **기존 필드가 그대로다**(Flutter 소비자)

**되돌리기 어려운 작업:** 없음(조회 전용). 단 **권한 slug 는 시드에 남으므로** 이름을 처음에 맞게 정할 것.

**Depends on:** 없음 (독립)

**Plans:** **4 plans / 3 waves** (2026-08-17 계획 완료)
- W1 — `80-01` 서버 집계·계약(cohort 단위 WIP + ETA 상태 + 균등 분배) · `80-02` 권한(필드 단위 차단)
- W2 — `80-03` 화면(마지막 2열 + `≈` 표기 + 권한 시 열 자체 제거)
- W3 — `80-04` 회귀·무결성(합계 보존 · 원장 복제 · 모집단 · 권한 우회 · 진단 지표)
...
### Phase 82: Enviado — 온라인 판매 배송 관제 보고서

**Goal:** Despacho 보드가 답하지 못하는 질문에 답하는 보고서를 만든다 —
**얼마나 걸렸나 · 어디서 늦나 · 어느 운송사가 느린가 · 길에 묶인 재고가 얼마인가.**
그리고 Reservado 보고서를 **숨긴다**(지우지 않는다).

**계기:** 사용자 요청(2026-08-17). *"Despacho 부분이 워낙 잘 만들어져 있어서 어떤 보고서가
Enviado(Online Venta) Control 메뉴에 적합할지 다른 시스템에서 아이디어를 얻고 싶다."*

**Mockup:** https://claude.ai/code/artifact/eec55483-c567-4fb3-889d-75a26059a103

**외부 조사에서 가져온 것** (Shopify · ShipStation · MercadoLibre 판매자 패널 · Shipium/DCL/Shipink):
상태가 아니라 **경과 시간으로 정렬** · "떠났는데 안 닿은 것"을 별도 탭 · **운송사별** 비교
(전체 평균은 특정 운송사의 부진을 감춘다) · KPI 옆에 **업계 기준치 표기**.
★ 조사가 권한 "KPI 3개" 중 **반품률은 뺐다** — `online_orders` 에 반품 상태가 없어
**항상 0 인 칸**이 된다. 대신 **En tránsito(건수 + 묶인 금액)** 를 넣는다.

**Requirements:**
- R1 **정의 고정** — `shipped_at` 을 Enviado 의 정본으로(★ `dispatched_at` 도 있어 섞으면 갈라진다)
- R2 **정시 기준 단일 출처** — `ON_TIME_MAX_DAYS = 5`(사용자 결정: 약속일 컬럼을 만들지 않는다)
- R3 **구간 분해** — confirmado→preparado→despachado→transporte→entregado 평균 일수.
  타임스탬프가 4개 있는 것이 이 시스템의 강점이다(조사한 시스템 대부분은 2점만 갖는다)
- R4 **화면이 기준을 말한다** — `meta 95%` · `criterio ≤ 5 días` · `8–10% ideal`
- R5 **Reservado 숨김** — registry 에 `hidden` 플래그. 코드·권한 슬러그는 그대로 남긴다

**★ 사용자 결정 (2026-08-17):** "정시"는 **내부 기준**(발송 후 5일)으로 한다.
`promised_delivery_date` 를 만들지 않는다 — 스키마 변경과 입력 경로가 따라온다.
→ 그 대신 **화면이 기준을 그대로 표기**해야 한다. 표기 없는 88% 는 좋은지 나쁜지 알 수 없다.

**범위 밖:** 약속 배송일 컬럼 · 반품률 · 요일·시간대 히트맵(주문 12건 — 무늬만 나온다) ·
운송사 API 연동(첫 배송 시도 성공률·추적 페이지 참여도) · Reservado 코드 삭제

**★ 측정 타당성 (2026-08-17 사용자 지적에서 나온 교정):** 운송사 API 연동이 **없다** —
`delivered_at` 은 직원이 Despacho 보드에서 누른 시각이다(`deliverOrder … by=${userName}`).
그대로 OTD 를 헤드라인으로 쓰면 **운송사 성과가 아니라 클릭 습관**을 재고, 아무도 안 누른 주문이
영원히 `En tránsito` 에 남아 금액을 부풀린다.
→ **주 KPI 를 `Sin confirmar`(확인 안 된 발송)로 바꾸고**, OTD 는 `Entregas confirmadas a tiempo`
로 이름을 바꿔 **"확인 시각 기준" 각주**를 단다. **확인율**을 품질 KPI 로 추가한다.
직원 확인 시 **도착일을 입력**받아(기본 오늘) 늦게 눌러도 지연이 부풀지 않게 한다.
이 보고서는 그래서 "운송사 성과 측정" 이 아니라 **확인 습관을 고치는 도구**로 시작한다.

**Success Criteria:**
- 주 KPI 가 시스템이 쓰는 값(`shipped_at`)만으로 계산된다
- OTD 에 "확인 시각 기준" 각주가 있다 (없으면 아무도 이게 클릭 습관인 줄 모른다)
- `Sin confirmar` 목록에서 **일괄 확인 + 도착일 입력**이 된다
- Enviado 와 "정시" 의 정의가 각각 **코드 한 곳**에서만 나온다
- 정시율 분모에 취소 주문이 들어가지 않는다
- 구간 분해가 타임스탬프 결측을 0 으로 세지 않는다(평균이 낮아진다)
- Reservado 가 목록·검색에서 사라지되 **권한과 코드는 남는다**
- En tránsito 0건일 때 화면이 고장처럼 보이지 않는다

**되돌리기 어려운 작업:** 권한 슬러그 `reporte-enviado` 시드(이름을 처음에 맞게 정할 것).

**Depends on:** 없음 (독립)

**Plans:** **2 plans / 2 waves** (2026-08-17 계획 완료)
- W1 — `82-01` 서버(정의 상수 · KPI · 구간 분해 · 4개 목록 · 권한)
- W2 — `82-02` 화면 + Reservado 숨김 (`autonomous: false` — 화면 확인 필요)

### Phase 83: 고객 배달 확인 링크 (Confirmá tu entrega)

**Goal:** 배달 완료를 **고객이 직접 확인**하게 해서, 지금 "직원의 클릭 습관" 인 `delivered_at` 을
실제 수령 사실에 가깝게 만든다.

**계기:** 사용자 제안(2026-08-17) — 고객에게 추적 링크를 보내 OK 를 받고, 잊으면 직원이 대행.

**★ 원안에서 뺀 것:** *"직원이 확인 → 그것에 대해 고객이 OK"* 의 마지막 단계.
고객이 첫 번째를 안 눌러서 직원이 대행하는 건데 그 결과를 다시 고객 OK 에 걸면 **주문이 계속
열린 채 남아 처음 문제로 돌아온다.**
→ **운영 종결과 증거 품질을 분리한다**: 직원 확인은 **즉시 종결**, 고객에게는 **통지만**
(*"등록했습니다. 아니면 알려주세요"*). 침묵 = 동의. 고객이 "안 받았다" 를 누르면 **`En disputa`**.

**Requirements:**
- R1 **도착일과 클릭 시각 분리** — `delivered_at`(도착일) / `delivered_confirmed_at`(클릭) /
  `delivered_confirmed_by`('customer'|'staff'). 늦게 눌러도 지연이 부풀지 않게
- R2 **추측 불가능한 토큰** + 만료 + 레이트 리밋. 공개 엔드포인트는 그 주문만 보여준다
- R3 **공개 확인 페이지** — 모바일 우선, 로그인 없이 탭 1회
- R4 **분쟁(`En disputa`)** — 조용히 쌓이지 않게 Despacho 보드와 Enviado 보고서 양쪽에 노출

**★ 자동 확인은 만들지 않는다:** 신규 주문은 `deliverOrder` 가 물류 마일스톤만 찍지만
**레거시 폴백(`mirrorSaleId == null`)은 여전히 매출·외상을 만든다**. 자동화가 그 경로를 건드리면
**아무도 안 누른 회계 기록**이 생긴다.

**범위 밖:** 운송사 API·멀티캐리어 애그리게이터(ShipNow·Zipnova) 연동 — 주문 12건 규모에서는
유지비가 효용을 넘는다. 월 수백 건이 되면 재검토.

**Success Criteria:**
- 직원이 도착일을 과거로 지정해도 클릭 시각과 분리돼 저장된다
- 잘못된·만료 토큰이 주문 존재 여부를 흘리지 않는다
- 같은 토큰 재확인이 멱등이다
- 직원 확인이 고객 응답을 기다리지 않는다
- 자동 확인 경로가 코드에 없다

**되돌리기 어려운 작업:** 공개 엔드포인트 신설(인증 없는 표면이 늘어난다) · 스키마 5개 컬럼.

**Depends on:** Phase 82 (보고서가 확인율·분쟁을 보여줄 자리)

**Plans:** **2 plans / 2 waves** (2026-08-17 계획 완료)
- W1 — `83-01` 스키마·토큰·공개 API·도착일 입력
- W2 — `83-02` 공개 확인 페이지 + WhatsApp 통지 + 분쟁 노출 (`autonomous: false`)
...
클라우드와 맞춰진다.** 범위는 사용자가 확정한 「판매 한 바퀴」다 —
카하 개시 → 상품검색 → 판매 → 코만다/라벨 인쇄 → 카하 마감.

**설계 근거:** `.planning/PLAN-2026-09-08-오프라인-영업.md` (CODEX 와 6개 문제 조율 — 합의 5 · 사용자 결정 1)
**직전 핸드오프:** `.planning/HANDOFF-2026-09-09.md`
**Depends on:** Phase 86

★ **A-1·A-2 는 이미 완료·배포됐다**(2026-09-08~09). 이 phase 는 A-3 부터다.
  `sales.offline_number` + `OFF-<ULID>` + 신뢰 경계 · 영수증 «NO FISCAL» + 손님 참조번호 ·
  동기화 시 재인쇄 금지(`suppressAutoPrint`). 마이그레이션 4건 로컬·운영 양쪽 적용됨.

---

#### 실측한 현재 상태 (재조사 불필요)

- `edge-agent/` (Phase 58) — 지점 PC 에 Node + PostgreSQL. 미러 pull · `POST /api/offline/sales` ·
  인쇄 · outbox. 엣지 DB 는 **`mirror_rows` 라는 범용 jsonb 테이블 하나**라
  미러 테이블 추가에 **엣지 스키마 변경이 필요 없다.**
- 프런트(`api.service.ts`)는 오프라인+엣지+플래그일 때 **`POST /sales` 와 인쇄 2개만** 엣지로 우회한다.
  **GET 은 전부 클라우드로 나가 실패한다 — 이것이 가장 큰 공백(B).**
- 오프라인 **신규 로그인 불가** — 미러가 `users.password` 를 일부러 제외(Phase 72-01).
  **기존 세션 복원만** 가능하다.
- 운영에 엣지 **1대**(coolsistema, agent id 17)만 등록. **2026-07-20 이후 오프라인** —
  원인은 자동 시작 부재로 보이고 `install.ps1` 5단계로 해결했으나 **미검증**이다.
- `edge-ticket.js` / `edge-ticket.service.ts` 가 **이미 있다** — A-3 은 새로 만들기가 아니라 실측부터.

#### 결정 (계획에서 확정 — 다시 논의하지 않는다)

| # | 결정 |
|---|---|
| ① | 오프라인 판매 식별자 = `offline_number` + 영수증 «NO FISCAL». 세무 순번은 동기화 때 서버가 채운다 |
| ② | 재고는 **거부하지 않고 conflict 로 수용** — 물건은 이미 나갔다. 음수 허용 + 검토 큐 |
| ③ | 전자영수증은 **pendientes → 복구 후 발급**(사용자 결정). CAEA 는 2단계 |
| ④ | **엣지가 클라우드와 같은 모양**의 엔드포인트를 낸다 — 프런트에 어댑터를 두지 않는다 |
| ⑤ | 마지막 완전 동기화 **72시간 이내**에만 오프라인 판매 허용 |
| ⑥ | 오프라인 판정 = 병렬 프로브 · 빠른 진입 · 느린 복귀(히스테리시스) |

★★ **③ 에는 기한이 있다.** ARCA COMPG v4.0 `CbteFch`: Concepto=1(Productos) 은 생성일 기준
  **±5일**이고 **제출 월을 넘을 수 없다**(Concepto 2·3 은 10일).
  **월말 정전이면 창이 5일보다 짧아진다** → pendientes 에 D-day 경보가 필요하다(E).

---

#### 작업 묶음 (의존 순서 — 순서를 바꾸면 검증이 불가능해진다)

**W0 — 배포 선결 (지금 막혀 있는 것)**
- [ ] `edge-agent-v0.1.0` 태그 없음 → 다운로드 페이지 Edge 링크가 **404**. 태그 push 로 CI zip 생성
- [ ] `install.ps1` **Windows 실행 미검증** — 괄호 균형만 확인했다(이 기계에 pwsh 없음). 첫 설치는 로그를 보면서
- [ ] print-agent 재배포(`v1.2.2`) — `printJobId` 관문 · edge-failover 누수 수정 · **영수증 날짜 파싱**(Phase 87 직전 수정)이 매장 PC 에 아직 없다
- [ ] 파일럿 엣지(coolsistema, agent 17) 되살리기 — B 웨이브 검증의 전제

**W1 — A-3/A-4 신뢰·멱등 (기존 코드 실측 먼저)**
- [ ] edge ticket 실측 → 온라인 로그인 직후 발급, 오프라인에서 그것으로 진입
- [ ] 엣지 write 멱등키 — 요청당 엣지 재시도는 **한 번만**, 쓰기 재시도는 멱등키가 있는 경로만

**W2 — 엣지 POS 계약 ★ 가장 큰 공백**
- [ ] 클라우드와 **같은 모양**의 조회 엔드포인트 ≈10개(POS 한 바퀴에 필요한 것)
- [ ] 카하 로컬 상태 — ★ **미러 변경으로 처리하면 안 된다.** 판매와 똑같이 로컬 원장 + outbox 작업이어야 하고
      **같은 카하의 작업 순서가 보존**돼야 한다
- [ ] outbox 순서·재시도

**W3 — 동기화**
- [ ] 재고 conflict 수용(②) · 판매/재고/결제/카하 **원자 적용** · 충돌 감사
- [ ] ★ 오프라인 op 의 `error` 는 양쪽에서 **종결**이다 — 한 번 떨어지면 판매가 영영 사라진다.
      원장 착지 실패가 배치 전체를 죽이지 않게 할 것

**W4 — 안전장치 (★ A→B 뒤에)**
- [ ] 72h 신선도 게이트(⑤) — 엣지 `/api/health` 가 freshness 를 싣고 프런트가 게이트
- [ ] 빠른 오프라인 판정(⑥) — `/auth/me` 타임아웃 12초 → 2초, `navigator.onLine` 은 힌트로만

**W5 — 세무**
- [ ] pendientes 기한 경보(5일 · 같은 달) + D-day 표시
- [ ] 오프라인 F10 게이트 — 지금은 `setFacturarSale(createdSale)` 가 **id=null 판매로 발급 모달을 연다**
- [ ] 오프라인 참조로 판매 찾기 — 뒤 6자는 하루 1,000건이면 0.05% 충돌.
      **전체 번호 정확일치** + 지점·영업일 안 접미사 검색이어야 한다

★★ **W4 만 먼저 붙이면 안 된다** — 빨리 오프라인에 들어가 봐야 GET 과 카하 API 가 없으면
  *더 빨리* 깨지는 화면에 도달할 뿐이다. W1→W2 가 먼저다.

#### MVP 에서 제외 (합의)

오프라인 신규 로그인 · 오프라인 CAE · MercadoPago 승인 · 외상 한도 변경 ·
판매 수정/취소의 교차단말 동기화 · 가격/상품 관리 · **CAEA**(2단계 — 정전이 잦거나 길어지면 올린다)
...
**Goal:** 매장에 온 **아무나**(계정 없이, 앱 없이) 라벨의 QR 을 찍으면
**어느 매장 · 어느 지점 · 무슨 상품**인지 사진과 가격으로 보이는 페이지가 열린다.
그 페이지 끝에서 두 갈래로 보낸다 — ① 이 매장의 **reseller 신청**,
② 「당신 매장에도 이 시스템을」 = **Ventago 잠재고객 수집**.

**Requirements**: 공개 상품 페이지 · 라벨 QR 도착지 복구 · reseller 신청 경로 ·
리드 수집 · 다중 테넌트 노출 통제
...
#### ★ 착수 전에 알아야 할 실측 (2026-09-16)

**이 phase 는 「QR 시스템을 만드는」 일이 아니다. 인쇄는 이미 되고 있고, 도착지가 없다.**

| 이미 있는 것 | 근거 |
|---|---|
| QR 인쇄 전 경로 | zebra-agent `^BQN,2,4^FDQA,…` + 자동 크기맞춤(`effectiveQrModule`) · 프론트 「QR DINÁMICO」 패널 · NUEVO/CAMBIO 델타 추적 |
| 인쇄 이력 | `qr_print_log` (branch_id · product_id · price_type_id · printed_price · printed_name) — 운영 **5행**, 최근 **2026-09-08**, branch 6 |
| 공개 서버렌더 HTML 패턴 | `GET /api/public/shop/:storeId/store` → **200** (다크 네이비+골드 단일 HTML, 같은 오리진에서 공개 카탈로그 API 호출) |
| 공개 카탈로그 API | `:storeId/products` · `:storeId/products/:slug` · `:storeId/categories` · `:storeId/theme` (전부 `@Public()`) |

| 없는 것 / 막힌 것 | 근거 |
|---|---|
| **QR 의 도착지** | QR 은 `${PUBLIC_WEB_URL}/m/stock?s={storeId}&p={parentProductId}` 인데 운영에서 **308 → 404**. 즉 **이미 인쇄돼 매장에 붙은 라벨이 지금 404 로 떨어진다** |
| 지점 정보 | URL 에 **branch 가 없다**(`s`·`p` 뿐). `qr_print_log` 는 지점을 알지만 QR 에 안 싣는다 |
| reseller 신청 화면 | 백엔드는 있다(`reseller` 스키마 · `POST reseller/auth/register` 는 `@Public()`) 그러나 **전부 0행 — 한 번도 안 쓰임**. 빌드되는 앱 목록에 reseller 포털이 **없다** |
| Ventago 리드 수집 | **아무것도 없다** — 새로 만들어야 한다 |

★★ **두 CTA 는 「막다른 CTA」가 되기 쉽다.** 이 저장소에는 없는 페이지로 보낸
전례가 기록돼 있다. 도착지가 **실제로 열리는지**를 검사로 못 박지 않으면
이 기능은 「버튼은 있는데 눌러도 아무 일 없음」이 된다.

★ `reseller` 는 **`public` 이 아닌 별도 스키마**다(`reseller.resellers` 등).
  전 테이블 훑는 검사·백업·purge 에서 빠지기 쉬운 자리다.
...
| # | 결정 | 실측 근거 / 귀결 |
|---|---|---|
| 1 | **ⓐ 인쇄 당시 가격유형을 따라간다** — 라벨과 화면이 항상 일치 | `qr_print_log.price_type_id` 를 되찾아 그 유형의 **현재 가격**을 보여준다 |
| 2 | **재고는 공개 안 함.** 판매원 전용 앱으로만 | 공개 페이지에서 수량 필드 자체를 내보내지 않는다 |
| 3 | **열거는 온라인 tienda 를 켠 매장만.** 안 켠 매장은 **찍은 그 상품만** | `stores.slug` 가 곧 공개몰 활성 플래그다 — 운영에서 **14개 매장 중 2개만** 있다(6=`cool`, 9=`stock`) |
| 4 | **Admin·Configuración 에 「구입자가 QR 로 가격을 볼 수 있게 할 것인가」 설정**, 기본값 **꺼짐** | `store_configs` 의 기존 boolean 명명(`use_*` · `commerce_auto_sync` · `vto_enabled`)을 따른다. 켜는 것이 매장의 의사표시다 |

★★★ **①은 ⓐ로 확정됐다. 그 전제를 실측으로 확인했다.**

  왜 base 가 아닌가: `PRECIO 1`(price_type 11)의 **가격 행 17건 중 14건이
  `products.price`(base)와 다르다.** 지금까지 인쇄된 5건은 그 상품에 PRECIO 1 행이
  **없어 base 로 폴백**해 우연히 일치했을 뿐이다. base 를 보여주면 언젠가
  **종이와 화면이 고객 앞에서 다른 가격**을 말한다.

  ⓐ의 구현 전제 (2026-09-16 확인):
  · `qr_print_log` 에 유니크 인덱스가 **있다** —
    `uq_qr_print_log_branch_prod_pt (branch_id, product_id, price_type_id)`.
    그래서 `bulkCreate(updateOnDuplicate)` 가 실제로 upsert 로 동작한다.
  · ★ 유니크 키에 `price_type_id` 가 들어 있어 **같은 상품이 가격유형별로 여러 행**이
    될 수 있다(현재는 0건이지만 구조상 가능). ⤷ 조회는 **`printed_at` 최신 1건**으로
    확정해야 한다. 「아무 행이나」 고르면 가격이 요일마다 달라 보인다.
  · ★ `qr_print_log` 에 **`store_id` 가 없다.** `product_id` 만으로 찾으면 테넌트를
    넘는다 — `products` 조인으로 매장을 반드시 강제할 것.
  · ★ **인쇄 기록이 없는 라벨**(로그 이전에 인쇄됐거나 유실)은 가격유형을 모른다.
    그때의 폴백을 정해야 한다(base 표시 / 가격 숨김). **폴백에서는 ⓐ의 일치 보장이
    성립하지 않는다** — 그 사실을 화면이 숨기지 말 것.

★ **③은 이미 인쇄된 라벨과 충돌하지 않는다.** 인쇄된 5건은 전부 store 6 이고
  그 매장은 slug(`cool`)가 있어 공개몰이 켜져 있다. 즉 연번 `p=` 노출이 허용되는
  쪽이다. 불투명 토큰은 **공개몰을 안 켠 매장**에만 필요하므로, 기존 라벨을
  죽이지 않고 도입할 수 있다.

★ ④의 설정이 **③보다 상위**다. 설정이 꺼져 있으면 공개몰 여부와 무관하게
  가격을 보여주지 않는다(상품명·사진까지만, 혹은 페이지 자체를 닫는다 — W0 에서 확정).

#### 제안 작업 순서

**W0 — 설정 스위치와 하위호환 (설계)**
- ④ `store_configs` 플래그 신설(예: `qr_precio_publico`), **기본값 꺼짐 확정**.
  **이것이 최상위 스위치**다 — 꺼져 있으면 공개몰 여부와 무관하게 가격을 안 보여준다.
  ★ 기존 매장 14개가 전부 꺼짐으로 시작하므로, **켜기 전까지 아무것도 바뀌지 않는다**
    (배포 안전). 무중단 규약대로 nullable/기본값으로 추가한다.
- ① **ⓐ 확정**. 구현 시 못 박을 것 셋: `printed_at` 최신 1건 · `products` 조인으로
  테넌트 강제 · **인쇄 기록 없는 라벨의 폴백** 정의(그때는 일치 보장이 없다).
- **URL 을 바꾸지 않는다**가 기본안 — 이미 붙은 라벨이 살아나는 것이 이 phase 의
  첫 가치다. 불투명 토큰은 **공개몰 미활성 매장 전용**으로 뒤에 더한다(③).
- 호스팅 방식 결정: `/m/stock` 은 **프론트 호스트**(app.coolsistema.com)다.
  ⓐ Next.js 공개 페이지(AclGuard 가 가로채지 않는지 확인 필요) ·
  ⓑ nginx 가 그 경로만 API 로 프록시 · ⓒ 그 외.

**W1 — 공개 상품 API** (`@Public()`)
- 정확히 필요한 필드만: 매장명·로고 · 지점명 · 상품명 · 사진 · 가격.
  **원가·공급처·내부 코드·재고수량은 내보내지 않는다** (②: 재고는 판매원 앱 전용).
- ④ 플래그가 꺼진 매장은 가격을 빼고 응답한다(또는 404 — W0 에서 확정).
- ③ 공개몰 미활성 매장은 **목록·카테고리·검색을 노출하지 않는다.** 찍은 그 상품만.
- 매장 비활성/삭제·상품 비활성 처리. throttle. 다중 테넌트 경계.
  ★ `@Public()` 은 테넌트 가드를 무력화한다 — `storeId` 를 코드에서 직접 강제할 것.

**W2 — 랜딩 페이지**
- 공개몰 스토어프론트와 같은 서버렌더 단일 HTML 패턴, 같은 테마.
- 사진 없음·가격 없음·상품 없음 각각의 화면을 정한다(빈 화면 금지).

**W3 — 지점을 QR 에 싣기 (하위호환)**
- 새 인쇄분에 `&b={branchId}` 추가. **`b` 가 없어도 페이지가 동작**해야 한다
  (이미 인쇄된 라벨). 없으면 매장 단위로만 표시.

**W4 — CTA ① 이 매장의 reseller 신청**
- 도착지부터 확인/구축: `POST reseller/auth/register` 가 실제로 도는가,
  신청 화면이 있는가, 신청이 **어느 매장**에 연결되는가(`reseller_tienda_link`).
- 0행 = 한 번도 안 쓰인 경로다. **실행된 적 없는 코드는 검증된 적 없는 코드**.

**W5 — CTA ② Ventago 잠재고객 수집**
- 새로 만든다. 최소: 연락처 + 매장명 + 어디서 왔는지(어느 매장의 QR 인지).
- **어디로 가는가**를 먼저 정한다(관리자 화면? 메일? 텔레그램?) — 받는 곳이
  없으면 수집은 사라진다.
...
- [x] 89-02-PLAN.md — 상품 이미지 파일명 모지바케 **진단만**(114건 중 60건). DB 바이트 ↔ MinIO 실재 키 대조로 ⓐ/ⓑ/ⓒ 판정. 고치지 않는다 [wave 1]
- [x] 89-10-PLAN.md — `prices (product_id, price_type_id)` UNIQUE(`CONCURRENTLY`·`NULLS NOT DISTINCT`) + `upsertPrices` → `ON CONFLICT` [wave 1] ★ 결정 ① 개정본의 「함께 확정된 것」
- [x] 89-03-PLAN.md — `qrPrecioPublico` 4벌 세트(모델 · FLAG_FIELDS · Context · Configuración 탭). 문구는 「상세 딥링크 허용」 성격 [wave 2]
- [x] 89-04-PLAN.md — 공개 QR API — 자격식 · 3갈래 · **3키 테넌트 강제** · `precioEtiqueta` 병기 · `labelMatch` · `storeApodo` [wave 2]
- [x] 89-05-PLAN.md — CTA② **이탈 받이**: `POST /public/ventago-leads` + 통지 **결과 기록**(`notify_status`) [wave 3]
- [x] 89-06-PLAN.md — 공개 페이지 `/m/stock` (상세 · 사진없음 · 목록전환 · 닫힘) + 가격 문구 3축 [wave 3]
...
- [x] 89-09-PLAN.md — **배포 게이트** + 운영 배포 승인 + **실물 라벨 스캔** + 두 CTA 끝까지 + `89-UAT.md` [wave 5]
...
**Goal:** 공개몰(virtual tienda)이 내보내는 **재고 수량**을 매장이 스스로 정할 수 있게 한다.
지금은 켜는 순간 수량이 숫자 그대로 공개되고 **끌 방법이 없다.**

**Requirements**: 매장별 재고 표시 설정 · 공개 응답 축소 · 기존 소비자 영향 검토

**Depends on:** Phase 89

#### 실측 (2026-09-16) — 이미 배포돼 돌고 있는 동작이다

```
GET https://newapi.coolsistema.com/api/public/shop/6/products   (인증 없음)
→ 200  {"id":140,...,"price":4300,"stock":21}
        {"id":264,...,"price":22000,"stock":369}
```

- `shop-catalog.service.ts` 가 `stock` 을 **항상** 내보낸다(104행; SQL 도 `COALESCE(m.available,0) AS stock` 무조건 SELECT).
- `stockClause`(158행)는 표시 여부가 아니라 **필터**다 — 재고 0 상품을 목록에서 뺄지.
  게다가 **요청 파라미터**이지 매장 설정이 아니다.
- `store_themes`(base_theme · macrostructure · tokens · enabled) · `store_configs` 어디에도
  재고 표시 스위치가 **없다**. `allow_sale_without_stock` 은 판매 허용이라 무관.
- 운영에서 공개몰을 켠 매장은 **2곳**(6=`cool`, 9=`stock`). store 6 은 실제 서비스 중.

★ Phase 89 의 결정 ②(재고는 판매원 앱 전용)와 **현재 동작이 어긋난다.**
  89 에서 조용히 고치지 않고 여기로 분리했다 — **이미 쓰이고 있는 응답을 바꾸는 일**이라
  소비자(공개몰 프론트·외부 연동) 영향 검토가 따로 필요하다.

#### 먼저 답해야 할 것

1. **누가 이 응답을 쓰고 있나.** 공개몰 스토어프론트 말고 다른 소비자가 있는가?
   (`stock` 을 지우면 그쪽이 깨진다 — 「무엇이 빠지나」를 세는 자리다)
2. **어떤 모드가 필요한가.** 숫자 그대로 / 「있음·없음」 / 완전 숨김 — 셋 다 필요한가?
3. **기본값.** 이미 공개 중인 2개 매장을 **그대로 둘 것인가**(하위호환) 아니면
   **숨김으로 바꿀 것인가**(그러면 지금 보이던 것이 사라진다 — 매장에 알려야 한다).
...
### Phase 93: 권한을 4단계로 — 메뉴 → 부메뉴 → 자원 → 동작 (0~6단계 완료, 6은 push 대기)

**진행 (2026-09-24):** 착수 계획은 `93-PLAN.md`(codex 자문 반영본) — 원안의 착수 순서를
바꿨다. 아래 요건 중 **역할 정리가 먼저** 나갔다(그게 4단계의 전제라서).

| 단계 | 상태 |
|---|---|
| **0** 선행 결함 5건 (P1-a~e) | ✅ 배포 (api #943, `f9df18a4`) |
| **1** 구 역할 생성 중단 (expand) | ✅ 배포 (api #944, `9a372b87`) |
| **2** 역할 데이터 이전 (migrate) | ✅ 운영·로컬 적용 (`2135e271`) |
| **3** 관찰 — JWT 최대 수명 이상 | ✅ 11시간 38분 · 403·500·perm거부 **0건** |
| **4** contract — alias 제거 + 구 역할 DELETE | ✅ 배포 (api #946 · front #784) + 마이그레이션 |
| **5** ①메뉴 → ②부메뉴 → ③기능 UI | ✅ 배포 (api #952 `0168acbf` · front #795 `1a2d6134`) + 마이그레이션 |
| **6** ④액션 UI (`[...ALL_ACTIONS]` 제거) | ✅ 구현·검증 완료 — **push 대기** |
| **7** 스페인어화 · 가짜 임계값 탭 제거 | ⏳ **여기** |
| **보류** | ~~동사형 132개 접기~~ — 별도 ADR 필요 |

★ 2단계에서 계획서의 「권한 합집합」을 **실측이 뒤집었다** — 합집합은 아무도 안 쓰던
  영구 삭제 권한을 실제 admin 에게 새로 준다. 사용자가 옮겨 가는 곳에서만 보장한다.

★ 3단계 관찰 실측: 이전(09-23 15:46 UTC) 후 **11시간 38분** > 로그인 토큰 최대 수명
  **6시간**. `/auth/me` 가 역할을 DB 에서 다시 읽어 구 slug 이월도 없다.
  이전된 `admin@noix`(store 19)가 이전 후 19:24 에 **실제로 판매**했다.

★ 4단계 마이그레이션(운영·로컬 양쪽 COMMIT): `roles` 48 · `role_functions` 8,155 ·
  `role_function_actions` 25,813(CASCADE — **계획서가 세지 않았던 것**). 백업 표 3개 보존.
  ⤷ 「`roles` 를 가리키는 FK」만 훑으면 안 보인다. 한 단계 건너뛴 참조는 카탈로그를
    한 번 훑는 것으로는 안 나온다.

★ 2단계가 권한을 하나 넓혔다(`branch_manager`→`gerente` 로 `ADMIN_ROLES` 진입).
  **위협이 아니다**(사용자 판단 2026-09-24): 구버전 시스템에 역할·권한 개념이 없어
  임포트가 가져올 권한이 없다. 다시 제안하지 말 것.
  ⤷ 남는 **방법**: 역할을 합칠 때 `role_functions` 만 대조하면 매트릭스 **밖**
    하드코딩 집합의 변화는 보이지 않는다.

★ 5단계 — 시드를 `admin` 앱에 두려다 **권한 확대**가 될 뻔했다(외부 검토 HIGH, 실측 확인):
  기능 1개 → 모듈 보임 → **앱이 `structure` 에 들어감**. 운영 120개 역할 중 **27개가
  `admin` 을 못 보는데**, `allowedApps={['admin']}` **하나로만** 열리는 화면이 **32개**다
  (`/configuracion/permisos` 포함). → 전용 앱 `herramientas`(아무 화면도 게이트로 안 씀,
  모듈이 전부 `is_auxiliary` 라 사이드바 그룹도 안 생김, 과금 대상 아님).
  ⤷ 「아무도 안 잃는다」는 **절반**이다 — **누가 얻는지**를 같이 세야 한다.

★ 6단계 — 가드 계약 실측: 호출부 214 · 고유 `(slug, action)` 102 · 가드 있는 기능 98 ·
  **액션 2개를 요구하는 기능은 4개뿐** · **가드가 아예 없는 기능 84**(메뉴 노출만).
  네 칸 고정이면 728칸 중 **626칸이 허수**다 → **기능마다 컨트롤 하나**, 모양은 가드가 정함.
  ⤷ 액션은 `GET /functions/acciones-de-guardia` 가 **판정하는 그 프로세스의 레지스트리**
    에서 준다. 이름에서 추론하지 않는다 — `editar-stock-de-producto` 는 `delete` 다.
  ⤷ 외부 검토 HIGH 2건 반영: 「켜기 ≠ 초기화」, 그리고 **숫자는 동시성 제어가 아니다**
    (`baseState` 전문 대조). 숫자 방어는 「덜 실려 옴」만 막는다.

**핸드오프:** `.planning/HANDOFF-2026-09-24-d-phase93-5y6-permisos.md`
(그 앞: `HANDOFF-2026-09-24-b-phase93-contract-y-envios.md`)
