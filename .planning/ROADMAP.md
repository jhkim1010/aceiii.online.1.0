...
- [ ] 37-06-PLAN.md — Wave 6: **QR 출퇴근 통제 (Fichaje/Asistencia)** — 신규 `seller_attendance` 세션테이블 + `attendance/` 백엔드 모듈(`GET /attendance/qr` caja 전용 SessionGuard HMAC 일일코드 + `POST /attendance/punch` 자동토글 entrada/salida, QR.store_id==seller.store_id 강제·크로스 지점 허용 + `GET /attendance/report` 월 총시간 + `PATCH /attendance/:id` 관리자 보정) + POS 웹 Ctrl+V 풀스크린 QR 오버레이(보조 모니터, 자정 store TZ 자동갱신) + Reportaje `reportes/asistencia` 페이지 + mobile-sales-app `attendance/` feature(scanner 재사용, "Fichar entrada/salida"). 설계: docs/superpowers/specs/2026-07-11-qr-attendance-fichaje-design.md. vendedor MVP(Wave 1-4)와 독립, revendedor 무관.
