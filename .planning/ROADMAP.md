...
- [ ] 37-06-PLAN.md — Wave 6: **QR 출퇴근 통제 (Fichaje/Asistencia)** — 신규 `seller_attendance` 세션테이블 + `attendance/` 백엔드 모듈(`GET /attendance/qr` caja 전용 SessionGuard HMAC 일일코드(**Windows/macOS 데스크톱 전용, 핸드폰 PLATFORM_NOT_ALLOWED 차단**) + `POST /attendance/punch` 자동토글 entrada/salida, QR.store_id==seller.store_id 강제·크로스 지점 허용 + `GET /attendance/report` 월 총시간 + `PATCH /attendance/:id` 관리자 보정) + POS 웹 Ctrl+V 풀스크린 QR 오버레이(보조 모니터, 자정 store TZ 자동갱신) + Reportaje `reportes/asistencia` 페이지 + mobile-sales-app `attendance/` feature(scanner 재사용, 단일 QR role 라우팅) + **`RequireAttendanceGuard`(vendedor 출근 전 카탈로그·스톡·판매 전면 차단 `NOT_CLOCKED_IN`, 퇴근·caja 마감 후 재차단)** + **caja cierre 훅(지점 열린 세션 강제 종료 → vendedor 완전 불능)** + **revendedor: 신규 `reseller_store_qr_auth`(매장 방문 승인 QR, 영구) + `RequireStoreAuthGuard`(Phase 24 관리자 승인 AND QR 물리확인 둘 다 필요, `STORE_NOT_AUTHORIZED`)**. 설계: docs/superpowers/specs/2026-07-11-qr-attendance-fichaje-design.md. vendedor 출퇴근이 37-06 실동작부, revendedor 매장승인 게이트 강제는 Phase 24 revendedor 엔드포인트와 함께 활성.
