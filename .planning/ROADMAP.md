...
**Requirements:** R1 복구 명령 채널(T1~T3, **T4 경로 없음**) · R2 복구 드릴(월 1회 자동) · R3 병렬 스위치(preflight·자동 복귀) · **R4 부하 생성기 ★** · R5 리허설 판정·게이트 자동 충전

**Depends on:** Phase 75 **W6-1**(advisory lock 리더) · **W6-3**(격리 플래그) · **W6-4**(replica 예산) — 이 셋 없이 켜면 리허설이 아니라 사고다. Phase 75 W1(`daily.jsonl`)은 리포트 저장소, W0-8(route p95)은 창 선정 근거.

**원안에서 교정한 전제 3가지:** ① **"주말엔 아무도 안 쓴다"는 미검증** — 아르헨티나 소매업은 토요일이 최대 매출일일 가능성이 높다. 창은 W0-8 요일별 실측(08-13)으로 정한다. ② **부하 없는 리허설은 아무것도 증명하지 못한다** — β·크론 중복·hot-row 는 전부 부하가 있어야 나타난다. 사용자 0명이면 전부 통과하고 **잘못된 확신**만 남는다. 그래서 R4 가 실질 핵심. ③ **스위치는 컨테이너 하나가 아니다** — 크론 리더·커넥션 예산·캐시 일관성·격리 플래그가 함께 움직여야 한다.
