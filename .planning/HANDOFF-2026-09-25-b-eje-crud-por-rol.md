...
1. ~~**NOIX 검색 화면**~~ → ✅ **2026-09-25 사용자 확인 완료.**
   `prices[0]` 결함(빈칸 15/174 · 틀린 가격 19건)과 ABC 정렬이 **화면에서 해소됐다.**
   ⤷ 이 건은 끝났다. 다음 세션에서 미해결로 다시 올리지 말 것.

2. **Permisos 역할 편집 서랍** (이번 세션 배포분) — ⬜ **미확인.**
   각 줄의 C/R/U/D 글자(꺼진 줄에도 흐리게) · `C R U D` 필터 칩 ·
   「Quitar «Eliminar» de N funciones」. Gerente 에서 `D` 를 누르면 **3개**가 남아야 한다
   (`Editar stock de producto` · `Gestionar Seña` · `Modificar venta`).

3. **Windows POS 의 `Alt+1` / `Ctrl+V`** (§4-2) — ⬜ **미확인.**

4. **Repaso 측정** (§4-5) — ⬜ 콘솔 한 줄.
