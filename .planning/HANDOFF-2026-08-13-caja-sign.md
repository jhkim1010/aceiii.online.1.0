...
# §B. Trello 오류 카드 — 2/3 종료 (2026-08-14)

두 세션 연속 "클라우드라 push 불가"로 보류돼 있던 카드들. 로컬이라 처리했다.

## B-1. `06TCj16i` 보류 판매 지점간 누수 — 완료 (api `d87bcc0` #699 / app `85e4127` #624 `dad6fdc` #625)

★ **카드만 믿지 말고 브라우저로 재현하라는 사용자 지적이 맞았다.** 재현이 카드보다 많은 것을 알려줬다.

**재현(coolsistema = 시험 매장)**: 지점 6 에서 보류 판매 #44 생성 → 사이드바에서
HELGUERA(16) 로 전환 → 목록에 #44 **그대로 남음**. 서버 로그도
`GET /api/suspended-sales` 파라미터 **0개**. 거를 코드 경로 자체가 없었다.

★ 단순 노출이 아니다 — 그 화면에서 복원하면 재고 hold 는 **지점 6** 에서 풀리는데
  판매는 16 에서 나가 **지점 간 재고가 소리 없이 이동**한다.

**수정**: `findByStore(storeId, branchId)`. 지점은 서버가 정하고 요청값은 검증만 한다.
정할 수 없으면 403 — 매장 전체로 되돌아가지 않고(누수 유지), 빈 배열도 아니다
("보류 판매가 없다"와 구분이 안 된다).

### ★ CODEX 가 구현 후 자문에서 잡은 구멍 2개
1. **`:id` 경로(조회/수정/삭제)가 storeId 만 봤다** — 목록만 막고 옆문을 열어 둔 셈.
   지점 B 사용자가 id 로 A 의 보류 판매를 지울 수 있었고 **A 지점 재고 원장**이 움직였다.
2. **생성 시 `dto.branchId` 소유 미검증** — 남의 지점으로 귀속 가능했다.

또 `resolveBranchId` 가 다중 지점 매장에서 조용히 `null` 을 돌려주던 것을 400 으로 바꿨다.
`null` 이면 `recordReservationMoves` 가 warn 만 남기고 **재고 hold 를 아예 기록하지 않는다** —
이번 누수의 근원이다.

### ★ 배포 후 로그에서 내가 만든 결함을 발견 (app `dad6fdc`)
`GET /api/suspended-sales 403 "No se pudo determinar la sucursal"` —
로드 직후 BranchContext 가 아직 안 잡힌 찰나에 파라미터 없는 요청이 나갔다.
인터셉터가 모든 에러를 빨간 배너로 띄우므로, 지점을 모르는 동안에는 **요청을 안 보낸다**.
→ **배포하고 끝내지 말고 운영 로그를 볼 것.**

### ★ 테스트가 엉뚱한 걸 검증하고 있었다
처음엔 "컨트롤러가 지점을 넘기는지"만 단언해 `:id` 뮤테이션이 **안 죽었다.**
where 절에 **실제로 들어갔는지**까지 봐야 죽는다([[test-can-pass-on-the-wrong-guard]]).
최종 34/34 + 뮤테이션 5종 사멸.

**검증(운영)**: HELGUERA → 0건(`?branchId=16`), coolsistema → #44 보임(`?branchId=6`).
전부 숨긴 게 아니라 제 지점에서만 보이는 것을 양쪽으로 확인했다.

## B-2. `OX5XFplw` 사이드바 라벨 2중 — 완료 (같은 커밋)

같은 화면에 `Cambiar sucursal` 이 두 번. 실제로는 **다른 동작**이었다:
· 지점 칩 → 보는 지점 변경 → `Ver otra sucursal`
· 금색 버튼 → 디바이스를 다른 지점 터미널로 재바인딩 → `Cambiar de terminal`
`sidebar_switch_terminal` 은 로케일에 **아예 없어** 하드코딩 fallback 이 쓰이고 있었다 —
es/en/ko 세 파일에 추가.

## B-3. `AkpJUsbI` REingreso 수량 유실 — **미착수**
`ProductsView.tsx:256-259` 그리드가 날짜 무관 전체 variant, `:300/322-327` 전부
`_isExisting:true` → 입력 수량이 **경고 없이 버려지고 성공 토스트**.
(진단은 `.planning/trello-inbox/report-2026-08-12.md`, 미검증)

## ★ B-4. 정리 안 한 것

- **보류 판매 #44 (coolsistema, 시험 데이터)** — 삭제 버튼이 `window.confirm` 을 띄우는데
  브라우저 다이얼로그는 Chrome 확장을 멈춘다 → 내가 못 누른다. **사람이 휴지통을 눌러야**
  재고 hold(`stocks#1800`, −1, type=suspend)가 정상 해제된다. 시험 매장이라 방치해도 무해.
- **store 3 의 레거시 NULL 행(#4)** — CODEX 는 백필을 권했으나, 그 건은 생성 당시 지점이
  없어 **재고 hold 가 기록되지 않았다**. 지금 지점을 채우고 나중에 삭제하면 `+1` 보정이
  들어가 **없던 재고가 생긴다.** 백필하려면 이 점을 감안해야 한다 — 손대지 않았다.
