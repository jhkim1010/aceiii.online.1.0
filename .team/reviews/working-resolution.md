...
# 라운드 3 (2026-08-20) — Trello 프론트 4건

대상: #262(변형 목록 액션) · #265(지점 전환 중복) · #266(침묵 실패) · #257(PDF 버튼 잘림)

**codex 판정: 이상 없음** (`006-round3-codex.md`). 수용/반박할 지적 없음.

## 이번 라운드에서 에이전트가 **하지 않기로** 한 것 — 둘 다 옳다고 판단

### #262 삭제 액션을 붙이지 않았다

변형 행 단위 취소 API 가 **존재하지 않는다.** 직접 대조했다:
`deleteStockTodayByParent(parentId, date, storeId)` 는 parentId 로 자식 전체를 찾아
그 자식들의 **모든 ProductBranch** 를 보정한다. 반면 목록 행은 `(제품 × 지점)` 단위다.
휴지통을 달았으면 "이 변형 이 지점만" 으로 읽히면서 형제 변형과 다른 지점 기록까지
지웠을 것이다 — 화면의 약속과 실제 동작이 어긋나는 형태다.

**요청받은 것을 그대로 만들지 않고 멈춘 판단이 맞다.**

### #257 전역 `overflow: hidden` 을 풀지 않았다

`VerticalLayout.tsx:142` 는 레이아웃 전역이다. 그것을 풀면 다른 화면에서 스크롤 사고가
난다. 리포트 영역 안에서 해결됐고(실측), 전역을 건드릴 이유가 없었다.

## 남은 불확실성 — 문서에 남긴다

**#266 은 원인을 규명하지 못했다.** 이번 변경은 "침묵을 없앤" 것이지 원인을 고친 것이
아니다. 요청이 **성공했는데** 응답에 `useCategory` 등이 없어 셀렉터가 안 뜨는 경우라면
새 Alert 도 뜨지 않는다(실패가 아니므로).

다음 제보 때 **"Alert 가 떴는지"** 만 물으면 두 경우가 갈린다 — 그것이 이번 변경의 값이다.

**#257 은 실제 앱 화면에서 확인하지 못했다.** 위 실측은 동일 CSS 구조의 격리 재현이다.
배포 후 브라우저로 확인해야 완료다.
