...
// 현재 로그인 매장 id (리포트/사용자 등 store-scoped 호출용).
final currentStoreIdProvider = Provider<int?>((ref) {
  return ref.watch(authControllerProvider).user?.storeId;
});
...
class AuthController extends StateNotifier<AuthState> {
  ...
  Future<void> bootstrap() async {
    try {
      ...
      // 복원된 유저가 매장 관리자 자격이 없으면 세션 무효화
      if (user != null && user.isStoreManager) {
        state = state.copyWith(user: user);
      }
    } catch (_) {
      // 복원 실패는 조용히 무시 → 로그인 화면
    }
  }
    ...
    try {
      ...
      if (!user.isStoreManager) {
        ...
        state = state.copyWith(
          loading: false,
          error: 'Acceso solo para administradores de tienda.',
        );
