        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              // 로그 전체 비우기 → 이후 새 항목만 보이게
              TextButton.icon(
                onPressed: () async {
                  final ok = await showDialog<bool>(
                    context: context,
                    builder: (c) => AlertDialog(
                      title: const Text('Limpiar consultas lentas'),
                      content: const Text(
                          'Se borrará todo el registro actual. Solo verás las nuevas desde ahora. ¿Continuar?'),
                      actions: [
                        TextButton(
                  ...
                  if (ok != true) return;
                  try {
                    await ref.read(diagnosticsRepositoryProvider).clearSlowQueries();
                    ref.invalidate(slowQueriesProvider);
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Registro de consultas lentas limpiado')),
                      );
                    }
                  } catch (e) {
                    if (context.mounted) {
                ...
                icon: const Icon(Icons.delete_sweep_outlined, size: 18, color: AppColors.dim),
                label: const Text('Limpiar', style: TextStyle(color: AppColors.dim, fontSize: 13)),
              ),
            ],
          ),
          const SizedBox(height: 8),
