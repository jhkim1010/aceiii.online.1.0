...
## ★★ 상시 규칙: push 는 묻지 말고 한다 (2026-09-25 확정 — 종전 규칙을 덮어씀)

> 사용자: 「작업이 웬만큼 마무리 되면 물어보지 말고 푸시하고 컴파일 해.. 위험한 경우가
>  아니면 승인을 기다릴 필요 없어. 푸시했다고 알려주기만 해」

**기본값은 push 다.** 검증이 끝나면 commit → push → 빌드·컨테이너 확인 → **사후 통보.**

- **commit 까지는 종전과 같다.** 변경 파일만 선별 스테이징(다른 WIP 오염 금지) → commit.
- **push 를 위해 멈추지 않는다.** 「push 할까요?」를 기본으로 묻지 않는다. 대신 보고에
  **무엇을 push 했고 빌드가 어떻게 됐는지**를 적는다.
- **여전히 승인이 필요한 것**(「위험한 경우」): 운영 DB 의 DML/DDL(SQL+영향 row 제시) ·
  서비스 재시작/중지 · 파괴적 명령 · 되돌리기 어렵거나 바깥으로 나가는 것.
- **로컬 검증(tsc · jest · eslint · 돌연변이)은 계속 한다.** 이 검증이 실제로 결함을 잡아낸다.

★ 왜 뒤집혔나: 승인을 기다리는 동안 **고쳐 놓은 것이 배포 안 된 채 남았다.**
  2026-09-25 POS 카트 수정이 커밋만 된 채였고 사용자가 「아직 벤타 푸시 안 했나?」로
  알아차렸다. 묻는 비용이 배포 지연으로 돌아왔다.
- 승인받아 push 한 뒤에는 종전대로 **빌드 결과를 반드시 확인한다** —
  `lastFailedBuild` 가 이번 빌드면 로그(`/var/lib/jenkins/jobs/<job>/builds/<n>/log`)를
  보고 수정한다. "push 완료"는 빌드 성공 + 컨테이너 재생성 확인까지를 의미한다.
  (커밋 SHA 로 내 빌드인지 확인한다 — 번호를 추측하지 않는다.)

★ **짝을 이루는 변경은 함께 배포한다.** api 와 app 이 서로를 전제하면 한쪽만 배포하는
  선택지를 만들지 말고, 어느 쪽이 먼저 나가도 안전한지를 **먼저 판단해서 알린다.**
  (2026-09-09 실례: 카하 지점 가드는 API 만 먼저 배포되면 옛 프론트가 400 을 받는다.)

### 규칙 변천 (참고)
- 2026-07-29: push 까지 Claude 가 직접 → 2026-09-09: 승인 필요 → **2026-09-25: 다시 직접**
  (위 상시 규칙). 「빌드가 곧 배포」라는 **사실은 변하지 않았으므로** 배포 확인 절차는 유지한다.
