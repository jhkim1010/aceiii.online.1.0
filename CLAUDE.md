...
- **commit 까지는 Claude 가 한다.** 변경 파일만 선별 스테이징(다른 WIP 오염 금지) → commit.
- **push 는 하지 않는다. 물어본다.** `git push origin main` 이 곧 Jenkins 빌드이고,
  빌드가 곧 운영 배포다. 그래서 **push 가 승인 지점**이다.
  커밋을 만들어 두고 "push 할까요?" 로 넘긴다.
- **로컬 검증(tsc · jest · eslint · 돌연변이)은 계속 한다.** 운영에 영향이 없고,
  이 검증이 실제로 결함을 잡아낸다. 승인이 필요한 것은 **Jenkins** 다.
...
### 종전 규칙 (2026-07-29 — 폐기)
- ~~push 까지 Claude 가 직접 수행한다~~ → 위 규칙으로 대체됨.
  이유: 자동 push 가 곧 자동 빌드·배포였고, 빌드 빈도가 사용자 의도보다 높았다.
